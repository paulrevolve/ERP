...
const Rates = ({ canEdit }) => {
  ...
  const handleCalculation = async () => {
    try {
      ...
      const response = await fetch(
        `${backendUrl}/Orgnization/CalculateForcastedRates`,
        { method: "POST" },
      );
  ...
  // } catch (e) {
  //   setError(e.message || "Failed to calculate rates");
  //   // toast.error("Could not calculate forcast rates")
  //   // await fetchData()
  //   // await fetchData()
  //   const response = await fetch(
  //     `${backendUrl}/Orgnization/CalculateForcastedRates`,
  ...
  //   await handleCalculation()
  //   window.location.href = "/dashboard/pool-rate-tabs";
  // };
  ...
  return (
    <div className="w-full bg-white p-6 space-y-2">
      {/* Top bar – follow Fringe style */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          <div className="flex items-center gap-2">
            <div className="input-label">Closing Period:</div>
            <div className=" input-label">{closePeriodLabel || "-"}</div>
          </div>
        ...
        <div className="flex items-center gap-1.5 bg-[#17414d] border border-blue-200 rounded-md px-2.5 py-1.5 shadow-sm text-xs">
          ...
          <span className="input-label text-white">All rates are in %</span>
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="overflow-x-auto border border-gray-200 rounded-lg max-h-[400px] bg-white">
          <table className="table w-full min-w-[900px]">
            <thead className="thead bg-gradient-to-r from-gray-50 to-gray-100 sticky top-0 z-20 border-b-2 border-gray-200">
              <tr>
                <th className="th-thead" rowSpan={2}>
                  Pool Number
                </th>
                <th className="th-thead" rowSpan={2}>
                  Pool Name
                ...
                {monthsHeader.map((m) => (
                  <th key={m} className="th-thead" colSpan={1}>
                    {m}
                  </th>
                ))}
              </tr>
              <tr>
                {monthsHeader.map((_, i) => {
                  ...
                  const closed = isPeriodClosed(period, selectedYear);
                  return (
                    <th key={period} className={"th-thead"}>
            ...
            <tbody className="tbody divide-y divide-gray-100">
              {data.map((row) => (
                ...
                >
                  <td className="tbody-td">{row.poolNumber}</td>
                  <td className="tbody-td ">{row.poolName}</td>
                  {row.months.map((m, i) => {
                    const period = i + 1;
                    ...
                    const closed = isPeriodClosed(period, selectedYear);
                    return (
                      <td key={period} className={"tbody-td"}>
                        {formatNumber(m.rate)}
                      </td>
                    );
                  })}
