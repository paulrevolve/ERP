...
const AnalogRate = () => {
  ...
  const categories = [
    ...
    { label: "2 - Sumaria Labor Onsite (LABOR)", key: "labOnste" },
    {
      label: "2 - Sumaria Labor Onsite (UNALLOW-LABOR)",
      key: "labOnsteNonBill",
    },
    { label: "5 - Sumaria Travel (NON-LABOR)", key: "nonLabTrvl" },
    { label: "6 - Subcontractors (LABOR)", key: "subLab" },
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      await fetch(`${backendUrl}/Orgnization/CalculateAnalogusRates`, {
        method: "POST",
      });
      const response = await fetch(`${backendUrl}/api/AnalgsRt`);
      if (!response.ok) throw new Error("Failed to fetch data");
  ...
  const initializeOverwriteRecord = (fyCd) => {
    const calculatedRec = rawData.find(
      (d) => d.fyCd === fyCd && d.actualAmt === false && d.ovrwrteRt === false,
    );
  ...
  const getValue = (categoryKey, fyCode, isActual) => {
    ...
    // Filtering and sorting by timeStamp to get the latest record
    const matchingRecords = rawData.filter(
      (d) =>
        d.fyCd === fyCode &&
        (isActual
          ? d.actualAmt === true
          : d.actualAmt === false && d.ovrwrteRt === targetStatus),
    );

    if (matchingRecords.length === 0) return "";

    const latestRecord = matchingRecords.sort(
      (a, b) => new Date(b.timeStamp) - new Date(a.timeStamp),
    )[0];

    return latestRecord ? (latestRecord[categoryKey] ?? "") : "";
  };
  ...
  const handleRateChange = (categoryKey, fyCode, newValue) => {
    setRawData((prevData) => {
      ...
      let index = newData.findIndex(
        (d) =>
          d.fyCd === fyCode && d.actualAmt === false && d.ovrwrteRt === true,
      );
      const parsedVal = newValue === "" ? 0 : parseFloat(newValue);
  ...
  const handleApplyToAll = (sourceYear) => {
    ...
    categories.forEach((cat) => {
      sourceValues[cat.key] = sourceRecord[cat.key] ?? 0;
    });
    ...
    setRawData((prevData) => {
      ...
      fyYears.forEach((year) => {
        let index = newData.findIndex(
          (d) =>
            d.fyCd === year && d.actualAmt === false && d.ovrwrteRt === true,
        );
        if (index > -1) {
          newData[index] = {
            ...newData[index],
  ...
  const handleSave = async (fyCode) => {
    const modifiedRecords = rawData.filter(
      (d) =>
        d.actualAmt === false && d.ovrwrteRt === true && d.isDirty === true,
    );
  ...
  if (loading)
    return (
      <div className="p-10 text-center text-gray-400 font-mono">Loading...</div>
    );
    ...
    <div className="w-full mx-auto bg-white p-4 rounded-xl shadow-lg border border-gray-100">
        ...
        <div className="flex items-center gap-6">
          <div className="flex items-center bg-gray-200 p-1 rounded-lg border border-gray-300 shadow-inner">
            <button
              ...
              className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${viewMode === "Calculated" ? "bg-white text-blue-600 shadow-sm" : "text-gray-500"}`}
            >
              Calculated
            </button>
            <button
              onClick={() => setViewMode("Overwrite")}
              className={`px-4 py-1.5 text-xs font-bold rounded-md transition-all ${viewMode === "Overwrite" ? "bg-white text-orange-600 shadow-sm" : "text-gray-500"}`}
            >
              Saved Overwrites
            </button>
          </div>
          <button
            onClick={fetchData}
            className="px-4 py-2 bg-white border border-gray-300 rounded-md text-sm font-semibold hover:bg-gray-100 shadow-sm"
          >
            Calculate Rate
          </button>
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full text-sm border-collapse">
          <thead>
            <tr className="bg-slate-100">
              <th className="border border-gray-300 px-3 py-3 text-left text-slate-700 min-w-[200px]">
                Category
              </th>
              <th className="border border-gray-300 px-3 py-3 text-right bg-blue-100/30 text-blue-900 min-w-[150px]">
                FY-2025 Actuals
              </th>
              {fyYears.map((year) => (
                <th
                  key={year}
                  className={`border border-gray-300 px-2 py-3 text-right ${editMode === year ? "bg-orange-50" : ""} min-w-[230px]`}
                >
                  <div className="flex flex-col items-end gap-2">
                    ...
                    <div className="flex flex-row gap-1 whitespace-nowrap">
                      {editMode === year ? (
                        <>
                          ...
                          <button
                            onClick={() => handleSave(year)}
                            className="bg-green-600 text-white px-2 py-1 rounded text-[10px] font-bold uppercase hover:bg-green-700"
                          >
                            {isSaving ? "..." : "Save"}
                          </button>
                          <button
                            onClick={handleCancel}
                            className="bg-gray-500 text-white px-2 py-1 rounded text-[10px] font-bold uppercase hover:bg-gray-600"
                          >
                            Cancel
                          </button>
                        </>
                        ...
                        viewMode === "Calculated" && (
                          <button
                            onClick={() => setEditMode(year)}
                            className="bg-blue-600 text-white px-3 py-1 rounded text-[10px] font-bold uppercase hover:bg-blue-700"
                          >
                            Overwrite
                          </button>
                        )
          ...
          <tbody>
            {categories.map((cat) => (
              <tr key={cat.key} className="hover:bg-gray-50/50">
                <td className="border border-gray-300 px-3 py-3 font-medium text-slate-700 text-xs">
                  {cat.label}
                </td>
                <td className="border border-gray-300 px-3 py-3 text-right financial-font font-semibold text-sm">
                  {getValue(cat.key, 2025, true) !== ""
                    ? `$ ${Number(getValue(cat.key, 2025, true)).toLocaleString()}`
                    : "—"}
                </td>
                {fyYears.map((year) => {
                  const isActive = editMode === year;

                  // Sorting records to find latest for this year
                  ...
                  const latestSavedRecord = relevantRecords.sort(
                    (a, b) => new Date(b.timeStamp) - new Date(a.timeStamp),
                  )[0];

                  // Use dirty record if available, otherwise latest saved
                  const record =
                    relevantRecords.find((r) => r.isDirty) || latestSavedRecord;
                  const isDirty = record?.isDirty;

                  return (
                    <td
                      key={year}
                      className={`border border-gray-300 px-1 py-1 transition-all ${isActive ? "bg-orange-50/20" : isDirty ? "pulse-update" : ""}`}
                    >
                      <input
                        type="number"
                        disabled={!isActive}
                        className={`w-full text-right px-2 py-2.5 rounded outline-none financial-font font-bold text-xl
                          ${
                            isActive
                              ? "text-orange-700 bg-white border-2 border-orange-300 shadow-sm"
                              : isDirty
                                ? "text-indigo-600"
                                : viewMode === "Overwrite"
                                  ? "text-blue-700"
                        ...
                        value={
                          isActive || isDirty
                            ? (record?.[cat.key] ?? "")
                            : getValue(cat.key, year, false)
                        }
                        onChange={(e) =>
                          handleRateChange(cat.key, year, e.target.value)
                        }
                      />
                    </td>
                  );
