...
const processData = (
  ...
  const totalEmployeeCount = new Set(actualsData.map((item) => item.emplId))
    .size;
  ...
  capacityData.forEach((item) => {
    ...
    capacityMap.set(
      monthKey,
      selectedEmployeeId === "All" ? baseHours * totalEmployeeCount : baseHours,
    );
  });

  const mappedActuals = actualsData
    .map((item) => {
      const dateObj = item.effectBillDt ? new Date(item.effectBillDt) : null;
      if (!dateObj || isNaN(dateObj)) return null;
  ...
  const filteredByFilters = mappedActuals.filter((item) => {
    const matchEmp =
      selectedEmployeeId === "All" || item.id === selectedEmployeeId;
    const matchYear =
  ...
  const avgDirect =
    labels.length > 0
      ? directLabor.reduce((a, b) => a + b, 0) / labels.length
      : 0;
  const avgCapacity =
    labels.length > 0
      ? totalCapacity.reduce((a, b) => a + b, 0) / labels.length
      : 0;
  const hoursAxisMax =
    Math.ceil((Math.max(...directLabor, ...totalCapacity, 10) * 1.15) / 10) *
    10;
...
const UtilizationChart = () => {
  ...
  const uniqueYears = useMemo(() => {
    const years = new Set(
      actualsData
        .map((item) => new Date(item.effectBillDt).getFullYear())
        .filter((y) => !isNaN(y)),
    );
    return [
      "All",
  ...
  const employeeOptions = useMemo(() => {
    const idsFromActuals = new Set(
      actualsData.map((item) => item.emplId).filter(Boolean),
    );

    const list = Array.from(idsFromActuals)
      .map((id) => {
  ...
  const handleRunReport = async () => {
    ...
    try {
      ...
      const validDates = actuals
        .map((i) => new Date(i.effectBillDt).getTime())
        .filter((t) => !isNaN(t));
      if (validDates.length > 0) {
        ...
        const capRes = await fetch(
          `${backendUrl}${AVAILABLE_HOURS_API}/${start}/${end}`,
        );
        const capacity = await capRes.json();
        setCapacityData(capacity);
      }
  ...
  const stats = useMemo(
    () =>
      processData(actualsData, capacityData, selectedEmployeeId, selectedYear),
    [actualsData, capacityData, selectedEmployeeId, selectedYear],
  );

  return (
    <div
      className="w-full bg-[#f8fafc] min-h-screen p-0 md:p-6"
    ...
    >
      ...
      <div className="w-full space-y-4">
          ...
          <div className="flex gap-4">
            {reportGenerated && (
              <button
                ...
                className="bg-slate-800 hover:bg-black text-white px-8 py-3 rounded-lg text-xs transition-all uppercase tracking-widest shadow-lg"
              >
                Export PDF
              </button>
            )}
            <button
              ...
              disabled={isLoading}
              className="bg-[#2563eb] hover:bg-[#1d4ed8] text-white px-8 py-3 rounded-lg text-xs transition-all uppercase tracking-widest shadow-lg disabled:bg-slate-300"
            >
        ...
        <div
          ref={reportRef}
          className="bg-white p-10 rounded-xl shadow-sm border border-slate-200 w-full overflow-hidden"
        >
          <div className="flex flex-col lg:flex-row justify-between items-start gap-8 mb-12 border-b border-slate-100 pb-8">
            <div>
              {/* <h2 className="text-3xl font-bold text-slate-900 tracking-tight mb-2 uppercase">Monthly Hours Comparison</h2> */}
              <p className="text-slate-500 italic">
                Actual hours vs.Available hours capacity
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 w-full lg:w-1/2">
              {/* Searchable Input */}
              <div className="relative" ref={dropdownRef}>
                <label className="text-[10px] uppercase text-slate-400 tracking-widest mb-1 block">
                  Employee ID & Name
                </label>
                <input
                  type="text"
                  value={searchTerm}
                  onFocus={() => {
                    if (!reportGenerated) setShowToast(true);
                    else setShowDropdown(true);
                  }}
                  onChange={(e) => {
                    setSearchTerm(e.target.value);
                    if (reportGenerated) setShowDropdown(true);
                  }}
                    ...
                    {employeeOptions
                      ...
                      .map((opt) => (
                        <div
                          ...
                          className="px-5 py-3 hover:bg-slate-50 cursor-pointer text-xs font-normal text-slate-600 border-b last:border-0"
                        >
                          {opt.label}
                        </div>
                      ))}
              ...
              <div>
                ...
                <select
                  value={selectedYear}
                  onChange={(e) => setSelectedYear(e.target.value)}
                  className="w-full p-3 bg-[#fdfdfd] border border-slate-200 rounded-lg outline-none focus:border-blue-500 text-slate-700 cursor-pointer font-normal"
                >
                  {uniqueYears.map((year) => (
                    <option key={year} value={year}>
                      {year === "All" ? "Full Period" : `FY ${year}`}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <Bar
                options={{
                  ...
                  plugins: {
                    legend: {
                      position: "bottom",
                      labels: { padding: 30, font: { size: 12 } },
                    },
                  },
                  scales: {
                    y: {
                      beginAtZero: true,
                      max: stats.hoursAxisMax,
                      grid: { color: "#f1f5f9" },
                    },
                    x: { grid: { display: false } },
                  },
                ...
                data={{
                  ...
                  datasets: [
                    ...
                    {
                      label: "AVG ACTUAL",
                      data: stats.labels.map(() => stats.avgDirect),
                      borderColor: "#00A389",
                      borderDash: [6, 4],
                      type: "line",
                      pointRadius: 0,
                    ...
                    {
                      label: "AVG CAPACITY",
                      data: stats.labels.map(() => stats.avgCapacity),
                      borderColor: "#FF7F50",
                      borderDash: [6, 4],
                      type: "line",
                      pointRadius: 0,
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8 pt-10 border-t border-slate-100 text-center">
            <div>
              ...
              <p className="text-3xl font-medium text-[#00A389]">
                {stats.avgDirect.toFixed(0)}{" "}
            ...
            <div>
              ...
              <p className="text-3xl font-medium text-[#FF7F50]">
                {stats.avgCapacity.toFixed(0)}{" "}
                <span className="text-sm font-normal">hrs</span>
              </p>
            </div>
            <div>
              ...
              <p className="text-3xl font-medium text-blue-600">
                {stats.avgCapacity > 0
                  ? ((stats.avgDirect / stats.avgCapacity) * 100).toFixed(1)
                  : 0}
                %
              </p>
            </div>
