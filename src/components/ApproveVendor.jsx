...
import { ActionDetailButton, FormSearchSelect } from "../helper/formSection";
import { TableSearchSelect } from "../helper/tableSection";

const ApproveVendor = ({ canEdit }) => {
  ...
  const COLUMN_LABELS = {
    vendApprvlCd: "Approve Code",
    vendId: "Vendor",
    vendName: "Vendor Name",
    vendGrpCd: "Vendor Group",
    payApprovalCode: "Approval Code Pay Vendor",
    payVendId: "Pay Vendor",
    payVendName: "Pay Vendor Name",
  };
  ...
  return (
    <div className="p-4 space-y-4 animate-in z-10 fade-in duration-500">
      <MainContainer icon={Building2} title="Approve Vendor">
        <Toolbar
          actions={{
            onSave: handleSave,
          }}
...
{
  /* <SecondaryContainer title="Approve Pay Vendors Options">
        <FormSection>
          <FormSearchSelect
            label="Pay Vendor Option"
            options={typeOptions.filter(
              (t) =>
                String(t.label)
                  .toLowerCase()
                  .includes(searchTermType.toLowerCase()) ||
                t.value.toLowerCase().includes(searchTermType.toLowerCase()),
            )}
            displayKey="label"
            secondaryKey="value"
            onSelect={(opt) =>
