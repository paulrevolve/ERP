...
const PLCMasterForm = ({ mode, selectedPLC, onClose, onSaveSuccess }) => {
  ...
  const [isUpdateMode, setIsUpdateMode] = useState(Boolean(selectedPLC)); // New state to manage update mode
  ...
  useEffect(() => {
    ...
    setForm({
      plcCode: selectedPLC.plcCode || "",
      description: selectedPLC.description || "",
      // costCom: selectedPLC.costCom || "",
      // comName: selectedPLC.comName || "",
      active: selectedPLC.active || false,
    });
  }, [selectedPLC]);
  ...
  const handleSave = async () => {
    if (!form.plcCode) {
      toast.error("PLC ID is required");
      return;
    }

    setIsSaving(true);
    const plcPayload = {
      plcCode: form.plcCode,
    ...
    try {
      ...
      if (isUpdateMode) {
        ...
        response = await api.put(
          `https://planning-master.onrender.com/api/PlcCodes/${plcPayload.plcCode}`,
          plcPayload,
        );
        ...
        response = await api.post(
          `https://planning-master.onrender.com/api/PlcCodes`,
          plcPayload,
        );
      ...
      const msg =
        error.response?.data?.message || "Failed to save plc details.";
      toast.error(msg);
    } finally {
      setIsSaving(false);
    }
        ...
        <form className="space-y-4">
          ...
          <div className="bg-white flex justify-between items-center pb-4 border-b-2 border-gray-300">
            <h2 className="text-[16px] font-medium text-gray-700">
              {isUpdateMode ? "Update PLC Master" : "PLC Master"}
            </h2>
            ...
            <div className="space-y-3 border border-gray-200 p-2 rounded bg-gray-50">
              <div className="label-input-div">
                <label className="input-label text-[10px]">
                  PLC ID <span className="text-red-500">*</span>
                </label>
                <input
                  name="plcCode"
                  value={form.plcCode}
