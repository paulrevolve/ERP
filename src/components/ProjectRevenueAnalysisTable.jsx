...
const ProjectRevenueAnalysisTable = ({ planId, style, onClose }) => {
        ...
        const correctedData = {
          ...
          employeeForecastSummary:
            res.data.employeeForecastSummary?.map((row) => ({
              ...row,
              burdenCost:
                row.totalBurdonCost ?? row.burdonCost ?? row.burdenCost ?? 0,
  ...
  const handleNew = () => {
    if (editingRowIndex !== null) {
      toast.warn(
        "Please save or close the current edit before adding a new row.",
      );
      return;
    }
    setInputRow(emptyRevenueAnalysisRow());
  ...
  const handleSave = () => {
    ...
    // api.post('/api/revenue-analysis-line', inputRow).then(...).catch(...)
  };
  ...
  const handleRowClick = (index) => {
    if (editingRowIndex !== null && editingRowIndex !== index) {
      toast.warn(
        "Please save or close the current edit before editing another row.",
      );
      return;
    }
    if (editingRowIndex === index) {
      // Clicked on the currently editing row
  ...
  const handleSelectAll = (e) => {
    if (e.target.checked) {
      const allRowIds = new Set(
        rows.map((row, idx) => row.emplId || `row-${idx}`),
      );
    ...
    if (!planId) {
      return (
        <tr style={{ height: ROW_HEIGHT_PX }}>
          <td
            colSpan={Object.keys(columnLabels).length + 1}
              ...
              {Object.keys(columnLabels).map((colKey) => (
                ...
                >
                  {isEditing ? (
                    ...
                    />
                  ) : colKey.includes("Revenue") ||
                    colKey.includes("Cost") ||
  ...
  return (
    <div className="p-4 font-inter">
      <div className="flex justify-between items-center mb-3">
        <h2 className="text-xs font-normal text-gray-800">Revenue Analysis</h2>
