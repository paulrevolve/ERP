...
import PLCMasterForm from "./PLCMasterForm";

const PLCMaster = ({ canEdit }) => {
  ...
  const [columns] = useState(["laborCategoryCode", "description", "active"]);

  const COLUMN_LABELS = {
    laborCategoryCode: "PLC ID",
  ...
  const toggleRow = (item) => {
    setSelectedRows((prev) => {
      ...
      if (newSet.has(item.id || item.laborCategoryCode)) {
        newSet.delete(item.id || item.laborCategoryCode);
      } else {
        newSet.add(item.id || item.laborCategoryCode);
      }
      return newSet;
    });
  };
  ...
  const toggleSelectAll = () => {
    if (isAllSelected) {
      ...
      setSelectedRows(new Set(data.map((d) => d.id || d.laborCategoryCode)));
    }
  };

  const handleDelete = async () => {
    if (!window.confirm(`Delete ${selectedRows.size} PLC(s)?`)) return;

    setIsDeleting(true);
    try {
      for (let id of selectedRows) {
        await api.delete(`${backendUrl}/api/PlcCodes/${id}`);
      }
  ...
  const handleEdit = () => {
    ...
    const PLCToEdit = data.find((item) => item.laborCategoryCode === id);
    setselectedPLC(PLCToEdit);
    setEditPopup(true);
  };
  ...
  const handleNewRow = () => {
    const newEntry = {
      ...
      displayId: "",
      laborCategoryCode: `TEMP_${Date.now()}`,
      description: "",
      active: false,
      isNew: true,
      isDirty: true,
  ...
  const handleCopy = () => {
    ...
    const selectedData = localData.filter((item) =>
      selectedRows.has(item.id || item.laborCategoryCode),
    );
    ...
    const columns = ["laborCategoryCode", "description", "active"];
  ...
  const handleLocalChange = (idOrCode, field, value) => {
    setLocalData((prev) =>
      prev.map((item) => {
        // Match by the temporary 'id' (for new rows) or the 'laborCategoryCode' (for existing)
        if (item.id === idOrCode || item.laborCategoryCode === idOrCode) {
          return {
            ...item,
  ...
  const handleExport = (apiResponse) => {
    ...
    try {
      ...
      ws["!cols"] = columns.map((col) => {
        ...
        if (col === "active") return { wch: 12 };
        return { wch: 20 };
      });

      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, ws, "PLC Master");

      const dateStr = new Date().toISOString().split("T")[0];
      XLSX.writeFile(wb, `PLC_Master_${dateStr}.xlsx`);
    ...
    try {
      ...
      reader.onload = async (event) => {
        ...
        jsonRows.forEach((row, index) => {
          const laborCategoryCode = String(row["PLC ID"] || "").trim();
          const description = String(row["Description"] || "").trim();
          const activeValue = String(row["Active"] || "")
            .trim()
            .toLowerCase();

          if (!laborCategoryCode) return;
          ...
          importedData.push({
            ...
            displayId: laborCategoryCode, // Used for the "ID" column input
            description: description,
        ...
        if (importedData.length === 0) {
          return toast.info("No valid PLC records found in file.");
        }
        ...
        for (const row of importedData) {
          try {
            const payload = {
              laborCategoryCode: row.laborCategoryCode,
            ...
            saveErrors.push(
              `${row.laborCategoryCode}: ${err.response?.data?.message || "Save Failed"}`,
            );
          }
        }
        ...
        if (successfulKeys.length > 0) {
          toast.success(
            `Saved ${successfulKeys.length} PLC records successfully.`,
          );
        }
  ...
  const handleBulkSave = async () => {
    ...
    if (newCount > 0 && updateCount > 0) {
      message = `Save ${newCount} new PLC entries and update ${updateCount} existing ones?`;
    } else if (newCount > 0) {
      message = `Save ${newCount} new PLC entry${newCount > 1 ? "s" : ""}?`;
    } else {
      message = `Update ${updateCount} PLC record${updateCount > 1 ? "s" : ""}?`;
    }
    ...
    if (window.confirm(message)) {
      ...
      const savePromises = rowsToSave.map((row) => {
        ...
        const plcCode = row.isNew ? row.displayId : row.laborCategoryCode;

        const payload = {
          laborCategoryCode: plcCode,
          description: row.description,
          active: row.active, // This is already a boolean from our Yes/No logic
          modifiedBy: user?.name || "system",
        };

        // Determine POST (New) or PUT (Update)
        const request = row.isNew
          ? api.post(`${backendUrl}/api/PlcCodes`, payload)
          : api.put(
              `${backendUrl}/api/PlcCodes/${row.laborCategoryCode}`,
              payload,
            );
        ...
        return request
          .then(() => ({
            ...
            key: row.id || row.laborCategoryCode,
          }))
          .catch((err) => ({
            status: "error",
            key: row.id || row.laborCategoryCode,
      ...
      try {
        ...
        setLocalData((prev) =>
          prev.filter(
            (row) => !successfulKeys.includes(row.id || row.laborCategoryCode),
          ),
        );
        ...
        if (successfulKeys.length > 0) {
          toast.success(
            `Successfully saved ${successfulKeys.length} PLC records.`,
          );
        }
  ...
  const handleFindReplace = () => {
    ...
    if (!window.confirm(`Apply changes to ${scope} PLC rows?`)) return;

    setLocalData((prev) => {
      return prev.map((item) => {
        const isSelected = selectedRows.has(item.id || item.laborCategoryCode);
        ...
        {showFormPopup && (
          <div className="bg-gray-50 p-4 border border-gray-300 rounded mb-4 shadow-sm animate-in fade-in slide-in-from-top-2">
            <div className="flex flex-wrap items-end gap-4 text-xs">
              ...
              {findReplaceConfig.column === "active" ? (
            ...
            <div className="fixed inset-0 z-50 flex items-center justify-center p-2 sm:p-4">
              ...
              <div className="relative bg-white w-full max-w-5xl h-fit max-h-[95vh] min-h-[55vh]  lg:max-h-[90vh] flex flex-col animate-premium-popup shadow-2xl rounded-lg overflow-hidden">
                <PLCMasterForm
                  ...
                  selectedPLC={editPopup ? selectedPLC : null}
                  onSaveSuccess={() => {
                    handleSearch();
                    setShowNewPopup(false);
                    setEditPopup(false);
              ...
              <tbody className="bg-white divide-y divide-gray-100">
                {isLoading ? (
                  ...
                  <tr>
                    ...
                    >
                      Search for an PLC to view the details.
                    </td>
                  </tr>
                ) : (
                  localData.map((item) => {
                    ...
                    return (
                      <tr
                        key={item.id || item.laborCategoryCode}
                        className={`${selectedRows.has(item.laborCategoryCode) || item.isNew ? "bg-blue-50" : ""}  hover:bg-gray-50 transition-colors`}
                      >
                        <td
                        ...
                        >
                          <input
                            type="checkbox"
                            // Use item.id if you implemented the unique temporary ID, otherwise item.laborCategoryCode
                            checked={selectedRows.has(
                              item.id || item.laborCategoryCode,
                            )}
                        ...
                        <td className="px-2 py-1 text-xs border-r border-b font-normal border-gray-300 text-gray-900 text-center">
                          <input
                            ...
                            onChange={(e) =>
                              handleLocalChange(
                                item.id || item.laborCategoryCode,
                                item.isNew ? "displayId" : "laborCategoryCode",
                                e.target.value,
                              )
                            }
              ...
              <div className="flex items-center justify-end gap-2 px-4 py-2  border-gray-100 w-fit ml-auto mb-1 text-sm text-gray-900">
                ...
                <div className="relative flex items-center rounded px-2 bg-white transition-colors">
                  ...
                  >
                    ...
                    <option value={70} className="text-black bg-white">
                      70 / page
                    </option>
