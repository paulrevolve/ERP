...
//     setLoading(true);
//     try {
//       const poolResponse = await api.get(
          ...
          {pools.length === 0 ? (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
              {pools.map((pool) => (
                <div
                  key={pool.poolId}
                  className="flex items-center space-x-3 p-4 border rounded-xl hover:shadow-md transition-all duration-200 bg-white"
                >
                  <input
                    type="checkbox"
                    id={pool.poolId}
                  ...
                  <label
                    htmlFor={pool.poolId}
                    className="text-sm font-semibold text-gray-900 cursor-pointer select-none flex-1"
                  >
