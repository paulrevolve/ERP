...
const MassLinkOrgReorg = () => {
  ...
  const handleGenerateLinks = () => {
    ...
    selectedAccIds.forEach((reorgId) => {
  ...
  const handleHeaderCheckboxChange = (e) => {
    if (e.target.checked) {
      const allIds = accounts.map((acc) => acc.reorgId);
  ...
  return (
    <div className="p-4 space-y-4 mt-10 animate-in z-10 fade-in duration-500">
      <div className="flex gap-2">
        <MainContainer icon={Building2} title="Reorganization">
                  ...
                  <tr className="tr-tbody">
                    <td className="tbody-td w-8">
                      <input
                        type="checkbox"
                        className="w-3 h-3 accent-blue-500"
                        checked={selectedAccIds.includes(acc.reorgId)}
                        onChange={() => handleRowCheckboxChange(acc.reorgId)}
                      />
                    </td>
                    <td className="tbody-td">{acc.reorgId}</td>
                    <td className="tbody-td">{acc.reorgName}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <tbody className="tbody">
              {linkedData.map((row, index) => (
                <tr key={index} className="tr-tbody">
                  {/* Read-Only Fields */}
                  <td className="tbody-td bg-gray-50">{row.reorgId}</td>
                  <td className="tbody-td bg-gray-50">{row.reorgName}</td>
                  <td className="tbody-td bg-gray-50">{row.orgId}</td>
                  <td className="tbody-td bg-gray-50">{row.orgName}</td>
...
export default MassLinkOrgReorg;
