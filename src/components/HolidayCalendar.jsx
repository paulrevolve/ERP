...
const AnnualHolidaysPage = ({ canEdit }) => {
  ...
  const handleDeleteRow = useCallback(async (holiday) => {
    if (holiday.id) {
      setIsLoading(true);
      try {
        await api.delete(`${API_BASE}/${holiday.id}`);
  ...
  const handleSaveHoliday = useCallback(
    async (holidayToSave) => {
      ...
      try {
        let saved;
        if (holidayToSave.id) {
          const res = await api.put(`${API_BASE}/${holidayToSave.id}`, payload);
          saved = res.data;
        ...
        <div className="bg-white p-4 w-full rounded-sm">
          ...
          <div className="flex items-center gap-3 w-full">
            ...
            <div className="flex items-center gap-2 mb-2">
              <button
                onClick={handleAddNewRow}
                disabled={isLoading || !canEdit("annualHolidays")}
