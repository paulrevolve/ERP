...
const PoolRate = ({ userName = "User", canEdit }) => {
  ...
  useEffect(() => {
    ...
    const fetchPoolsAndRates = async () => {
      setLoading(true);
      try {
        const poolsResponse = await api.get(
          ...
          <div className="gap-4 flex w-full items-center ">
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-2">
                ...
                <select
                  id="template"
                  className="border outline-none border-gray-300 rounded px-2 py-1.5 text-xs sm:text-sm bg-white shadow-inner w-[200px]"
                    ...
                    <div className="mt-20 w-full max-w-md bg-white rounded-lg shadow-xl border">
                      <div className="mt-3 space-y-3 bg-gray-50 p-4 rounded-lg shadow-sm">
                        <div className="flex items-center justify-between px-2 ">
                          ...
                          <button
                            type="button"
                            className="bg-gray-200 text-gray-700 cursor-pointer p-1 rounded "
          ...
          <div className="">
            {selectedTemplate && selectedYear && (
              <div>
                ...
                <div className="border border-gray-300 rounded-lg shadow overflow-x-auto ">
                  <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="th-thead ">Month</th>
                        {/* <th className="th-thead ">Statutory & 401K</th>
                        <th className="th-thead ">Indirect Lab IT/Fac Alloc</th>
                        <th className="th-thead ">Grant Off-site</th>  */}
                        {pools.map((pool) => (
                          <th
                            key={pool.poolId}
                            className={`th-thead ${pool.groupName.toLowerCase() === "hr" ? "uppercase" : ""}`}

                            // className="th-thead uppercase w-[60px]"
                          >
                            {/* {pool.groupName} */}
                            {pool.groupName.toLowerCase() === "hr"
                      ...
                      {monthlyData.length > 0 && !loading && !error ? (
                        monthlyData.map((data) => (
                          <tr key={data.month} className="bg-white">
                            <td className="tbody-td ">{data.month}</td>
                            ...
                            {pools.map((pool) => (
                              <td key={pool.poolId} className="tbody-td ">
                                {/* <input
                                  type="text"
