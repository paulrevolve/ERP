...
// //   Calendar, UserCheck, Coins, Globe, Receipt, ChevronRight, ChevronDown, ChevronUp,
// //   Printer, Share2, Paperclip, Tag, UserPlus, Bell, HelpCircle, Command, ArrowUpDown,
// //   Check, CheckCircle2, AlertCircle, Plus
// // } from "lucide-react";
...
// //   <div className={`bg-white border border-slate-200/90 rounded-xl p-3.5 shadow-2xs relative ${className}`}>
// //     {title && (
// //       <span className="text-[11px] font-bold text-slate-700 block mb-2 tracking-wide uppercase">
// //         {title}
// //       </span>
// //     )}
// //     {children}
...
// //   <div className="p-1.5 bg-slate-100 border border-slate-200 rounded-lg shadow-sm -mr-2.5 flex items-center justify-center">
...
// //         {label && (
// //           <label className={`text-[11px] font-semibold text-slate-600 select-none shrink-0 ${labelClassName || "min-w-[120px]"}`}>
...
// //               className="w-3.5 h-3.5 cursor-pointer accent-[#17414d]"
// //             />
// //             Yes
// //           </label>
// //           <label className="flex items-center gap-1.5 cursor-pointer text-xs font-semibold text-slate-800 select-none">
...
// //           onChange={onChange}
// //           disabled={actualDisabled}
// //           className="w-4 h-4 rounded-full border-slate-300 text-slate-900 cursor-pointer accent-slate-900"
// //         />
// //         {label && (
// //           <label className={`text-xs font-medium text-slate-700 cursor-pointer select-none ${labelClassName}`}>
// //             {label} {required && <span className="text-rose-500 font-bold">*</span>}
// //           </label>
// //         )}
// //       </div>
...
// //         {label && (
// //           <label className="text-[11px] font-semibold text-slate-600 min-w-[110px] text-left select-none">
// //             {label} {required && <span className="text-rose-500 font-bold">*</span>}
// //           </label>
// //         )}
// //         <input
...
// //           className={`w-full text-xs font-semibold px-3 py-2 rounded-lg transition-all outline-none border flex-1 min-w-0 shadow-2xs ${inputClassName}
// //             ${actualReadOnly || actualDisabled
// //               ? "bg-[#f1f3f5] border-slate-200 text-slate-500 cursor-not-allowed"
// //               : "bg-[#f3f4f6] border-slate-200/90 hover:bg-[#e9ebef] hover:border-slate-400 focus:bg-white focus:border-[#17414d] focus:ring-2 focus:ring-[#17414d]/10 text-slate-900 placeholder:text-slate-400 cursor-text"
// //             }`}
// //         />
// //       </div>
// //     );
...
// //       {label && (
// //         <label className="text-[11px] font-semibold text-slate-600 select-none flex items-center gap-0.5 whitespace-nowrap">
// //           {label} {required && <span className="text-rose-500 font-bold">*</span>}
// //         </label>
// //       )}
// //       <input
...
// //         disabled={actualDisabled}
// //         placeholder={placeholder}
// //         className={`w-full text-xs font-semibold px-3 py-2 rounded-lg transition-all outline-none border min-w-0 shadow-2xs ${inputClassName}
// //           ${actualReadOnly || actualDisabled
// //             ? "bg-[#f1f3f5] border-slate-200 text-slate-500 cursor-not-allowed"
// //             : "bg-[#f3f4f6] border-slate-200/90 hover:bg-[#e9ebef] hover:border-slate-400 focus:bg-white focus:border-[#17414d] focus:ring-2 focus:ring-[#17414d]/10 text-slate-900 placeholder:text-slate-400 cursor-text"
// //           }`}
...
// //   return (
// //     <div className={`flex flex-col gap-1 w-full relative min-w-0 ${className}`}>
// //       {label && (
// //         <label className="text-[11px] font-semibold text-slate-600 select-none flex items-center gap-0.5 whitespace-nowrap">
// //           {label} {required && <span className="text-rose-500 font-bold">*</span>}
// //         </label>
...
// //             className={`w-full text-xs font-semibold pl-3 pr-8 py-2 rounded-lg transition-all outline-none border min-w-0 shadow-2xs
// //               ${disabled
// //                 ? "bg-[#f1f3f5] border-slate-200 text-slate-500 cursor-not-allowed"
// //                 : "bg-[#f3f4f6] border-slate-200/90 hover:bg-[#e9ebef] hover:border-slate-400 focus:bg-white focus:border-[#17414d] focus:ring-2 focus:ring-[#17414d]/10 text-slate-900 placeholder:text-slate-400 cursor-text"
// //               }`}
// //           />
// //           <div
// //             className="absolute right-0 px-2.5 cursor-pointer text-slate-400 hover:text-slate-600 transition-colors"
...
// //           <>
// //             <div className="absolute left-0 top-full z-[100] w-full mt-1.5 bg-white border border-slate-200 rounded-xl shadow-2xl max-h-48 overflow-y-auto custom-scrollbar p-1">
// //               {filteredOptions.length > 0 ? (
// //                 filteredOptions.map((opt, idx) => (
// //                   <div
// //                     key={idx}
// //                     className="px-3 py-2 text-xs hover:bg-slate-50 rounded-lg cursor-pointer transition-colors font-medium text-slate-800 flex items-center justify-between"
// //                     onClick={() => {
// //                       onSelect(opt);
// //                       setSearchVal("");
...
// //                       <span className="text-slate-400 text-[11px] ml-2">({opt[secondaryKey]})</span>
// //                     )}
// //                   </div>
// //                 ))
// //               ) : (
...
// //         tempId,
// //         voucher: "",
// //         voucherKey: undefined,
// //         isDirty: true,
// //         detailLines: (item.detailLines || []).map((d, dIdx) => ({
// //           ...d,
// //           id: `PASTE_D_${Date.now()}_${dIdx}`,
// //           voucherLnKey: undefined,
// //           voucherLnVendorKey: undefined
// //         }))
// //       };
// //       return updateHeaderBalances(record);
// //     });
...
// //     if (selectedRow.voucher) {
// //       const isDuplicate = records.some(r =>
// //         getRowKey(r) !== getRowKey(selectedRow) &&
// //         String(r.voucher).trim().toLowerCase() === String(selectedRow.voucher).trim().toLowerCase()
// //       );
// //       if (isDuplicate) {
// //         return toast.error("Voucher number already exists! Duplicate voucher number is not allowed.");
// //       }
// //     }
...
// //           </div>
// //         ) : (
// //           <FormSection title="Vendor Labor" className="px-2.5 py-2">
// //             <div className="flex flex-wrap items-start gap-2.5 py-1">
...
// //           </div>
// //         ) : (
// //           <FormSection title="Line Amounts" className="px-2.5 py-2">
// //             <div className="max-w-[410px] space-y-1 py-1">
// //               <FormInput
// //                 label="Cost Amount"
// //                 type="number"
...
// //               />
// //               <div className="space-x-2 flex items-center m-0.5">
// //                 <label className="f-head font-[400] text-[10px] text-black min-w-[82px] text-left select-none">
// //                   Taxability
// //                 </label>
...
// //                       }
// //                     }}
// //                     className="border outline-none px-1.5 py-1 rounded font-light text-[10px] transition-all w-full border-gray-300 bg-white focus:border-[#17414d]"
// //                   />
...
// //             <FormSection title="Value Added Tax Information" className="px-2.5 py-2">
// //               <div className="grid grid-cols-1 md:grid-cols-[140px_135px_190px] gap-2 py-1">
...
// //               </div>
// //             </FormSection>
// //             <FormSection title="Customs Header Information" className="px-2.5 py-2">
// //               <div className="grid grid-cols-1 md:grid-cols-[190px_190px_160px_190px] gap-2 py-1">
...
// //               </div>
// //             </FormSection>
// //             <FormSection title="Customs Line Information" className="px-2.5 py-2">
// //               <div className="grid grid-cols-1 md:grid-cols-[80px_165px_135px_130px_145px] gap-2 py-1">
...
// //                 <div className="col-span-1 flex flex-col gap-1 w-[120px]">
// //                   <label className="text-[11px] font-semibold text-slate-600 select-none flex items-center gap-0.5 whitespace-nowrap">
// //                     Goods/Services <span className="text-rose-500 font-bold">*</span>
// //                   </label>
// //                   <select
// //                     className="w-full text-xs font-semibold px-3 py-2 rounded-lg transition-all outline-none border min-w-0 shadow-2xs bg-[#f3f4f6] border-slate-200/90 hover:bg-[#e9ebef] hover:border-slate-400 focus:bg-white focus:border-[#17414d] focus:ring-2 focus:ring-[#17414d]/10 text-slate-900 cursor-pointer"
...
// //               <FormSection title="Voucher Header" className="px-2.5 py-2">
...
// //                   <div className="w-full sm:w-[68px]">
// //                     <FormSearchSelect
// //                       label="Fiscal Year *"
// //                       className="w-full"
...
// //                   <div className="w-full sm:w-[50px]">
// //                     <FormSearchSelect
// //                       label="Period *"
// //                       className="w-full"
...
// //                   <div className="flex flex-wrap items-center gap-x-2 gap-y-1 pb-0.5">
// //                     <FormInput
// //                       label="Approved"
// //                       type="checkbox"
// //                       className="w-[75px]"
...
// //               {/* TABS HEADER */}
// //               <div className="flex items-center gap-6 border-b border-slate-200 mb-2 px-1 select-none overflow-x-auto">
// //                 {[
// //                   { name: "Header Info" },
// //                   { name: "Details" },
// //                   { name: "Payment" }
...
// //                       className={`pb-2 text-xs font-semibold cursor-pointer transition-all relative whitespace-nowrap ${isActive
// //                         ? 'border-b-2 border-[#17414d] text-[#17414d] font-bold'
...
// //                       <div className="h-full flex flex-col md:col-span-1">
// //                         <FormSection className="h-full px-2.5 py-2" title="Invoice & Discounts">
// //                           <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
// //                             <div className="space-y-1.5">
// //                               <span className="text-[10px] font-bold text-slate-700 block mb-1">Invoice Details</span>
// //                               <FormInput
// //                                 label="Number"
...
// //                               />
// //                             </div>
// //                             <div className="space-y-1.5 border-t sm:border-t-0 sm:border-l border-slate-300 pt-2 sm:pt-0 sm:pl-2.5">
// //                               <span className="text-[10px] font-bold text-slate-700 block mb-1">Discounts Info</span>
// //                               <FormInput
// //                                 label="Percent"
...
// //                         <FormSection className="h-full px-2.5 py-2" title="Due & Routing Accounts">
// //                           <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
// //                             <div className="space-y-1.5">
// //                               <span className="text-[10px] font-bold text-slate-700 block mb-1">Due Information</span>
// //                               <FormInput
// //                                 label="Date"
// //                                 type="date"
...
// //                             </div>
// //                             <div className="space-y-1.5 border-t sm:border-t-0 sm:border-l border-slate-300 pt-2 sm:pt-0 sm:pl-2.5">
// //                               <span className="text-[10px] font-bold text-slate-700 block mb-1">Routing Accounts</span>
// //                               <FormSearchSelect
// //                                 label="A/P"
...
// //                     <div className="grid grid-cols-1 xl:grid-cols-[1fr_2.4fr] gap-3 items-start">
// //                       {/* Subcontractor Information (Row 1, Column 1) */}
// //                       <div className="order-1 md:order-none h-full">
// //                         <FormSection title="Subcontractor Information" className="h-full px-2.5 py-2">
// //                           <FormInput
...
// //                           />
// //                           <div className="flex flex-col gap-1 w-full sm:max-w-[160px] m-0.5">
// //                             <span className="f-head font-semibold text-[11px] text-slate-600 select-none">Invoice Type</span>
// //                             <select
// //                               className="border outline-none px-3 py-2 rounded-lg text-xs font-semibold bg-[#f3f4f6] border-slate-200/90 hover:bg-[#e9ebef] hover:border-slate-400 focus:bg-white focus:border-[#17414d] focus:ring-2 focus:ring-[#17414d]/10 transition-all w-full text-slate-900 cursor-pointer shadow-2xs"
...
// //                       {/* Payment Address (Row 1, Column 2 & 3 & 4) */}
// //                       <div className="order-3 md:order-none h-full">
// //                         <FormSection title="Payment Address" className="h-full px-2.5 py-2">
// //                           <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-[repeat(3,minmax(0,1fr))] gap-2 w-full">
// //                             <FormInput
// //                               label="Address Code"
...
// //                       {/* Address Selection (Row 2, Column 1) */}
// //                       <div className="order-2 md:order-none h-full">
// //                         <FormSection title="Address Selection" className="h-full px-2.5 py-2">
// //                           <FormSearchSelect
// //                             label="Pay Vendor"
...
// //                       {/* Notes (Row 2, Column 2 & 3 & 4) */}
// //                       <div className="order-4 md:order-none h-full">
// //                         <FormSection title="Notes" className="h-full flex flex-col justify-between px-2.5 py-2">
// //                           <div className="space-y-1.5 flex flex-col h-full justify-between max-w-[520px]">
// //                             <FormInput
...
// //                               helperText="Text prints on Blank Laser Checks only."
// //                             />
// //                             <div className="space-y-1 flex flex-col flex-1 justify-end">
// //                               <textarea
// //                                 className="w-full flex-1 min-h-[60px] border outline-none px-3 py-2 rounded-lg text-xs font-semibold bg-[#f3f4f6] border-slate-200/90 hover:bg-[#e9ebef] hover:border-slate-400 focus:bg-white focus:border-[#17414d] focus:ring-2 focus:ring-[#17414d]/10 transition-all resize-none text-slate-900 shadow-2xs"
...
// //                     {/* Totals Section */}
// //                     <div className="flex flex-wrap gap-3 justify-end items-center pt-3 border-t border-slate-100">
// //                       <div className="px-3.5 py-2 bg-[#f3f4f6] rounded-lg border border-slate-200/80 flex items-center gap-3 shadow-2xs">
// //                         <div className="flex flex-col">
// //                           <span className="text-[10px] font-semibold text-slate-500 select-none">Total Tax</span>
// //                           <span className="text-xs font-bold text-slate-900">${selectedRow?.totalTax || "0.00"}</span>
// //                         </div>
// //                       </div>
// //                       <div className="px-3.5 py-2 bg-[#f3f4f6] rounded-lg border border-slate-200/80 flex items-center gap-3 shadow-2xs">
// //                         <div className="flex flex-col">
// //                           <span className="text-[10px] font-semibold text-slate-500 select-none">Remaining Balance</span>
// //                           <span className="text-xs font-bold text-slate-900">${selectedRow?.remainingBalance || "0.00"}</span>
// //                         </div>
// //                       </div>
// //                     </div>
// //                   </div>
...
// //                 {activeTab === "Details" && (
// //                   <div className="space-y-3">
// //                     <div className="grid grid-cols-1 xl:grid-cols-[1fr_1.4fr] gap-3 items-start">
// //                       <div className="space-y-3">
// //                         <FormSection title="VAT Info" icon={FileSpreadsheet} className="px-2.5 py-2">
// //                           <FormSearchSelect
// //                             label="Tax ID"
...
// //                         <FormSection title="Retainage" className="px-2.5 py-2">
// //                           <FormInput
// //                             label="Rate"
...
// //                         <FormSection title="Reference Only" className="px-2.5 py-2">
// //                           <FormInput
// //                             label="PO"
...
// //                       <div className="space-y-3">
// //                         <FormSection title="Recur Voucher Info" className="px-2.5 py-2">
...
// //                                 <div className="grid grid-cols-5 gap-1.5 items-center text-[10px] font-medium text-slate-400 text-center select-none pb-1">
// //                                   <div></div>
// //                                   <div>FY</div>
// //                                   <div>Period</div>
// //                                   <div>Subperiod</div>
...
// //                                   <input
// //                                     type="text"
// //                                     className="w-full px-2.5 py-1.5 border border-slate-200/90 outline-none rounded-lg text-xs font-semibold text-slate-900 bg-[#f3f4f6] hover:bg-[#e9ebef] hover:border-slate-400 focus:bg-white focus:border-[#17414d] focus:ring-2 focus:ring-[#17414d]/10 transition-all shadow-2xs"
...
// //                                   <input
// //                                     type="text"
// //                                     className="w-full px-2.5 py-1.5 border border-slate-200 outline-none rounded-lg text-xs font-medium text-slate-400 bg-slate-100 cursor-not-allowed"
// //                                     value={selectedRow?.recurStartEndingDate || ""}
// //                                     disabled
// //                                   />
// //                                 </div>

// //                                 <div className="grid grid-cols-5 gap-1.5 items-center pt-1">
...
// //                         <FormSection title="Template" className="px-2.5 py-2">
// //                           <FormInput
// //                             label="Template"
...
// //                         <FormSection title="Entry Info" className="px-2.5 py-2">
// //                           <FormInput
// //                             label="User"
...
// //                 {activeTab === "Payment" && (
// //                   <div className="space-y-3">
// //                     <FormSection title="Vendor Details" className="px-2.5 py-2">
// //                       <div className="grid grid-cols-1 md:grid-cols-[1.2fr_2fr_1.5fr] gap-2.5">
// //                         <FormSearchSelect
// //                           label="Pay Vendor"
// //                           value={selectedRow?.payVendor || ""}
// //                           options={[{ value: "VND-1001", label: "VND-1001" }]}
...
// //                     <div className="grid grid-cols-1 xl:grid-cols-[1.3fr_1.2fr_0.9fr] gap-3 pt-1 items-start">
// //                       <FormSection title="Payment Information" className="px-2.5 py-2">
// //                         <FormInput
...
// //                       <FormSection title="Transaction Rules" className="px-2.5 py-2">
// //                         <div className="flex flex-col gap-1 w-full sm:max-w-[160px] m-0.5">
// //                           <span className="f-head font-semibold text-[11px] text-slate-600 select-none">Payment Transaction Type</span>
// //                           <select
// //                             className="border outline-none px-3 py-2 rounded-lg text-xs font-semibold bg-[#f3f4f6] border-slate-200/90 hover:bg-[#e9ebef] hover:border-slate-400 focus:bg-white focus:border-[#17414d] focus:ring-2 focus:ring-[#17414d]/10 transition-all w-full text-slate-900 cursor-pointer shadow-2xs"
...
// //                       <FormSection title="Period to Post" className="px-2.5 py-2">
// //                         <FormSearchSelect
// //                           label="Fiscal Year"
...
// //               className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-slate-200 bg-slate-50 hover:bg-slate-100 text-slate-700 font-semibold text-xs shadow-xs transition-all cursor-pointer select-none"
// //             >
// //               <Calculator size={13} className="text-slate-500" />
// //               <span>View Voucher Totals</span>
// //             </button>
// //           </div>
// //         </div>
// //       </MainContainer>
...
// //           isDiscard={!(selectedRow?.detailLines || []).some(d => d.isDirty)}
...
// //             <div className="space-y-3">
// //               {/* CHILD TABS HEADER */}
// //               <div className="flex items-center gap-6 border-b border-slate-200 mb-3 px-2 select-none overflow-x-auto">
// //                 {[
// //                   { name: "Account Info" },
// //                   { name: "Other Info" },
// //                   { name: "Line Notes" }
...
// //                       className={`pb-2.5 text-xs font-semibold cursor-pointer transition-all relative whitespace-nowrap ${isActive
...
// //               <FormSection className="px-2.5 py-2">
// //                 {!selectedChildRow ? (
// //                   <div className="text-center text-gray-400 italic text-[11px] py-8">
...
// //                       <div className="space-y-3">
// //                         <div className="grid grid-cols-1 xl:grid-cols-[620px_360px] gap-3">
// //                           {/* Left Column */}
// //                           <div className="space-y-3">
// //                             <FormSection title="Line & Allocations" className="px-2.5 py-2">
// //                               <div className="grid grid-cols-1 md:grid-cols-[95px_minmax(280px,430px)] gap-2.5 items-end">
// //                                 <FormInput
// //                                   label="Line No"
...
// //                               <div className="grid grid-cols-1 md:grid-cols-[160px_minmax(280px,430px)] gap-2.5 items-end pt-1">
// //                                 <div>
// //                                   <FormSearchSelect
// //                                     label="Account"
...
// //                             <FormSection title="Cost Calculations" className="px-2.5 py-2">
// //                               <div className="grid grid-cols-1 sm:grid-cols-[150px] gap-2">
// //                                 <FormInput
// //                                   label="Cost Amount"
// //                                   type="number"
// //                                   value={selectedChildRow.costAmount || ""}
...
// //                               <div className="grid grid-cols-1 sm:grid-cols-3 gap-2.5 pt-1 max-w-[460px]">
// //                                 <FormInput
...
// //                             <FormSection title="References" className="px-2.5 py-2">
// //                               <div className="grid grid-cols-1 md:grid-cols-[150px_minmax(260px,410px)] gap-2.5 items-end">
// //                                 <div>
// //                                   <FormSearchSelect
// //                                     label="Ref No 1"
...
// //                               <div className="grid grid-cols-1 md:grid-cols-[150px_minmax(260px,410px)] gap-2.5 items-end pt-1">
// //                                 <div>
// //                                   <FormSearchSelect
// //                                     label="Ref No 2"
...
// //                           {/* Right Column */}
// //                           <div className="space-y-3">
// //                             <FormSection title="Segment Abbreviations" className="px-2.5 py-2">
...
// //                             <FormSection title="Sales/VAT Info" className="px-2.5 py-2">
// //                               <div className="space-y-1.5">
...
// //                                         }
// //                                       }}
// //                                       className="w-full px-1.5 py-1 rounded border text-[10px] font-medium bg-white border-slate-200 text-slate-800 outline-none hover:border-slate-300 focus:border-slate-400 focus:ring-1 focus:ring-slate-400 transition-all"
// //                                     />
...
// //                             <FormSection title="Use/Reverse Info" className="px-2.5 py-2">
// //                               <div className="space-y-1.5">
// //                                 <FormInput
...
// //                         <div className="grid grid-cols-1 md:grid-cols-[360px_220px] gap-3">
// //                           <FormSection title="Vendor 1099's" className="px-2.5 py-2">
// //                             <div className="space-y-1.5">
// //                               <FormInput
// //                                 label="1099"
// //                                 type="checkbox"
...
// //                           <FormSection title="CIS" className="px-2.5 py-2">
...
// //                         <FormSection title="VAT Info" className="px-2.5 py-2">
// //                           <div className="grid grid-cols-1 md:grid-cols-[280px_160px] gap-2.5 pt-1">
// //                             <FormSearchSelect
// //                               label="Supply Code"
...
// //                         <span className="f-head font-[400] text-[10px] text-black select-none block">Notes</span>
// //                         <textarea
// //                           className="w-full h-28 border outline-none p-2 rounded font-light text-[10px] transition-all border-gray-300 bg-white focus:border-[#17414d]"
...
// //                 className="flex items-center px-2.5 py-1 rounded-md border border-slate-200/80 bg-slate-50 hover:bg-slate-100 text-slate-700 font-bold text-[11px] shadow-sm transition-all cursor-pointer select-none"
// //               >
// //                 {tab.name}
// //               </button>
// //             );
...
//   MoreVertical, X, Search, FileText, FileSpreadsheet, MapPin, 
//   CreditCard, Repeat, ShieldCheck, Settings, RefreshCw, Calculator, User, Layers,
//   Calendar, UserCheck, Coins, Globe, Receipt, Plus, Printer, MoreHorizontal, Save, Copy,
// ClipboardPaste
// } from "lucide-react";
// import { MainContainer, Toolbar, SecondaryContainer } from "../helper/container";
// import { ReusableTable } from "../helper/tableSection";
...
//   if (!records.length || !selectedRow) return;
...
//   setSelectedIds(new Set([getRowKey(nextRecord)]));

//   const firstLine = nextRecord.detailLines?.[0] || null;
//   setSelectedChildRow(firstLine);
//   setSelectedChildIds(
//     firstLine ? new Set([getRowKey(firstLine)]) : new Set()
//   );
// };
...
//         .payment-voucher-page .voucher-panel > .space-y-1\.5, .payment-voucher-page .voucher-panel > div:not(.flex.items-center) { padding:12px; }
...
//         .payment-voucher-page .voucher-content-shell { border:1px solid #dbe3eb; border-radius:8px; background:#fff; padding:14px; box-shadow:0 1px 2px rgba(31,41,55,.03); }
//         .payment-voucher-page .voucher-content-shell input:not([type="checkbox"]):not([type="radio"]), .payment-voucher-page .voucher-content-shell select { min-height:32px; }
//         .payment-voucher-page .voucher-secondary-trigger { margin:0 16px; border-top:1px solid #dbe3eb; border-bottom:1px solid #dbe3eb; background:#fff; padding:0 4px; }
//         .payment-voucher-page .voucher-secondary-tabs { display:flex; align-items:stretch; min-height:44px; overflow-x:auto; }
//         .payment-voucher-page .voucher-secondary-tab { position:relative; height:44px; padding:0 15px; border:0; background:transparent; color:#63758b; font-size:11px; font-weight:600; white-space:nowrap; }
//         .payment-voucher-page .voucher-secondary-tab:hover { color:#1677e8; background:#f7f9fc; }
//         .payment-voucher-page .voucher-secondary-tab.active { color:#1677e8; }
//         .payment-voucher-page .voucher-secondary-tab.active:after { content:""; position:absolute; left:10px; right:10px; bottom:-1px; height:2px; background:#1677e8; border-radius:2px 2px 0 0; }
//         .payment-voucher-page .voucher-second-screen { margin:0 16px 24px; padding:14px; border:1px solid #dbe3eb; border-top:0; background:#f7f9fc; }
//         .payment-voucher-page .voucher-second-screen > .rounded-xl { border-radius:8px !important; border-color:#dbe3eb !important; box-shadow:0 1px 2px rgba(31,41,55,.03) !important; }
//         .payment-voucher-page .voucher-second-screen .voucher-detail-content-shell { border:1px solid #dbe3eb; border-radius:7px; background:#fff; padding:14px; }
//         .payment-voucher-page .voucher-second-screen .voucher-nested-tabs { display:flex; align-items:center; gap:0; padding:8px 4px 4px; border-top:1px solid #eef1f4; }
//         .payment-voucher-page .voucher-second-screen .voucher-nested-tab { position:relative; display:inline-flex; align-items:center; height:28px; padding:0 12px; border:0 !important; border-radius:0 !important; background:transparent !important; color:#1a73e8 !important; font-size:12px; font-weight:500; box-shadow:none !important; }
//         .payment-voucher-page .voucher-second-screen .voucher-nested-tab:first-child { padding-left:0; }
...
//         .payment-voucher-page .voucher-secondary-tabs { min-height:26px; align-items:center; gap:0; overflow-x:auto; padding-left:16px; }
//         .payment-voucher-page .voucher-secondary-tab { position:relative; height:auto; padding:0 12px; color:#1a73e8; font-size:12px; font-weight:500; }
//         .payment-voucher-page .voucher-secondary-tab:first-child { padding-left:0; }
//         .payment-voucher-page .voucher-secondary-tab:hover { color:#125bc3; background:transparent; }
//         .payment-voucher-page .voucher-secondary-tab.active { color:#1a73e8; font-weight:600; }
//         .payment-voucher-page .voucher-secondary-tab.active:after { display:none; }
//         .payment-voucher-page .voucher-secondary-tab + .voucher-secondary-tab:before { content:"|"; position:absolute; left:0; color:#d0d7e2; font-weight:400; }
//         .payment-voucher-page .voucher-second-screen { margin:0 16px 24px; padding:6px 0 24px; border-top:1px solid #e5e7eb; background:#fff; box-sizing:border-box; }
//         .payment-voucher-page .voucher-second-screen-closebar { display:flex; align-items:center; justify-content:flex-end; min-height:34px; padding:0 4px; border-bottom:1px solid #e5e7eb; background:#fff; }
//         .payment-voucher-page .voucher-second-screen-close { display:inline-flex; align-items:center; justify-content:center; gap:5px; height:28px; padding:0 9px; border:1px solid #d5dfeb; border-radius:6px; background:#fff; color:#52657c; font-size:11px; font-weight:600; }
//         .payment-voucher-page .voucher-second-screen-close:hover { background:#f5f8fb; border-color:#b9c8d8; color:#344a63; }
//         .payment-voucher-page .voucher-first-screen-summary { display:flex; align-items:center; justify-content:space-between; gap:12px; min-height:44px; padding:5px 16px 5px; border-top:1px solid #e5e7eb; background:#fff; }
//         .payment-voucher-page .voucher-summary-actions { display:flex; align-items:center; justify-content:flex-end; gap:10px; margin-left:auto; }
//         .payment-voucher-page .voucher-summary-card { display:flex; align-items:center; gap:7px; min-height:28px; padding:4px 9px; border:1px solid #dbe3eb; border-radius:4px; background:#fff; }
//         .payment-voucher-page .voucher-summary-card span { font-size:10px; color:#52657c; font-weight:500; white-space:nowrap; }
//         .payment-voucher-page .voucher-summary-card strong { font-size:11px; color:#26364d; font-weight:700; white-space:nowrap; }
//         .payment-voucher-page .voucher-total-link { display:inline-flex; align-items:center; gap:5px; min-height:28px; padding:0 9px; border:1px solid #dbe3eb; border-radius:4px; background:#fff; color:#344a63; font-size:11px; font-weight:600; box-shadow:0 1px 2px rgba(31,41,55,.04); }
//         .payment-voucher-page .voucher-total-link:hover { background:#f7f9fc; border-color:#c7d3df; }
//         .payment-voucher-page .voucher-content-shell { padding-bottom:0 !important; }
//         .payment-voucher-page .voucher-content-shell .space-y-4 { row-gap:10px !important; }
//         .payment-voucher-page .voucher-second-screen > .rounded-xl { margin-top:0 !important; border:1px solid #e5e7eb !important; border-radius:6px !important; box-shadow:none !important; background:#fff !important; }
//         .payment-voucher-page .voucher-second-screen .voucher-detail-content-shell { border:0; border-radius:0; padding:0; background:#fff; }
//         .payment-voucher-page .voucher-second-screen .mt-4.flex.flex-wrap.gap-2 button { border:0; border-radius:0; background:transparent; color:#1a73e8; box-shadow:none; padding-left:8px; padding-right:8px; }
//         .payment-voucher-page .voucher-second-screen .mt-4.flex.flex-wrap.gap-2 button:hover { color:#125bc3; background:transparent; }
//         .payment-voucher-page .voucher-second-screen .rounded-xl > div:first-child { border-bottom:1px solid #e1e3e6 !important; background:#fff !important; }
//         .payment-voucher-page .voucher-second-screen .rounded-xl h2 { font-size:18px !important; font-weight:500 !important; color:#26364d !important; }
//         .payment-voucher-page .voucher-second-screen .rounded-xl { font-size:12px; }
...
//         .payment-voucher-page .voucher-master-card .flex.gap-4.pt-1 { padding-top:0 !important; }
//         .payment-voucher-page .voucher-master-card .flex.gap-4.pt-1 > div { flex:1; }
//         .payment-voucher-page .voucher-master-card .flex.gap-4.pt-1 label { width:100%; }
...
//         @media (max-width:900px) { .payment-voucher-page .voucher-head-btn { padding:0 9px; } .payment-voucher-page .voucher-tab { padding-left:10px; padding-right:10px; } }
//         .payment-voucher-page .voucher-chevron {
//   font-size: 12px;
//   line-height: 1;
//   display: inline-flex;
//   align-items: center;
...
//         /* FINAL ALIGNMENT: every first/second-screen outer edge uses the same parent width. */
//         .payment-voucher-page .new-ui-content { width:100%; min-width:0; box-sizing:border-box; }
//         .payment-voucher-page .voucher-master-card,
//         .payment-voucher-page .voucher-tabs,
//         .payment-voucher-page .voucher-content-shell,
//         .payment-voucher-page .voucher-first-screen-summary,
//         .payment-voucher-page .voucher-secondary-trigger,
//         .payment-voucher-page .voucher-second-screen {
//           width:100%;
//           max-width:none;
...
//         }
//         .payment-voucher-page .voucher-second-screen-closebar {
//           width:100%;
//           box-sizing:border-box;
//           min-height:34px;
//           padding:0 4px;
...
//         }
//         .payment-voucher-page .voucher-second-screen > .rounded-xl {
//           width:100% !important;
...
//           padding-right:0 !important;
//         }
//         .payment-voucher-page .voucher-nav-btn {
//   display: inline-flex;
//   align-items: center;
...
// .payment-voucher-page .voucher-nav-btn:hover {
//   background: #f5f8fc;
//   color: #1677e8;
// }
//   .payment-voucher-page .voucher-count {
//   min-width: 42px;
//   text-align: center;
//   color: #52627a;
//   font-size: 11px;
...
//       {/* NEW UI TOP BAR */}
//       <div className="h-[50px] border-b border-[#e5e7eb] bg-white px-5">
//         <div className="flex h-full items-center justify-between gap-4">
//           <div className="flex min-w-0 items-center gap-2 text-[11px] text-[#7b8798]">
//             <span className="font-medium text-[#4f5f76]">Accounting</span><span>›</span>
//             <span className="font-medium text-[#4f5f76]">Accounts Payable</span><span>›</span>
...
//       {/* NEW UI PAGE HEADER */}
//       <div className="border-b border-[#dbe3eb] bg-white">
//         <div className="flex items-center justify-between gap-4 px-5 py-3">
//           <div className="flex items-center gap-3 min-w-0">
//             <h1 className="text-[19px] font-semibold tracking-[-0.2px] text-[#172b4d] whitespace-nowrap">A/P Voucher</h1>
...
//     onClick={() => handleVoucherNavigation(-1)}
//   >
//     ←
//   </button>
...
//             <button type="button" onClick={handleAdd} className="voucher-primary-btn"><Plus size={14} /> Create <span className="voucher-chevron">⌄</span></button>
...
//               <style>{`
//                 .new-ui-content .relative.rounded.border { background:#fff !important; border:1px solid #dbe3eb !important; box-shadow:none !important; border-radius:7px !important; }
//                 .new-ui-content .relative.rounded.border > span.absolute { background:#fff !important; color:#344a63 !important; font-weight:600 !important; font-size:11px !important; }
//                 .new-ui-content .z-30.overflow-visible h2 { font-weight:700 !important; color:#1f2f44 !important; }
//               `}</style>

//               {/* Accounts Payable Vouchers Master Header Form Container */}
//               <div className="voucher-panel voucher-master-card mb-4">
//                 <div className="voucher-panel-title">
//                   <span className="text-[11px] font-bold text-slate-700 block mb-1">Accounts Payable Vouchers</span>
//                 </div>
...
//               {/* TABS CONTAINER */}
//               <div className="voucher-content-shell">
...
//                               </span>
//                               <textarea
//                                 className="w-full h-[58px] min-h-[58px] border border-slate-200 outline-none p-1.5 rounded text-[11px] font-medium bg-white focus:border-slate-400 focus:ring-1 focus:ring-slate-400 transition-all resize-none"
//                                 value={selectedRow?.notesText || ""}
//                                 onChange={(e) => handleFieldChange(activeRecordId, "notesText", e.target.value)}
//                               />
//                             </div>
...
//       {/* COMPACT TOTALS / FIRST-SCREEN FOOTER */}
//       <div className="voucher-first-screen-summary">
//         <div className="voucher-summary-spacer" />
//         <div className="voucher-summary-actions">
//           <div className="voucher-summary-card">
//             <span>Total Tax</span>
//             <strong>${selectedRow?.totalTax || "0.00"}</strong>
//           </div>
//           <div className="voucher-summary-card">
//             <span>Remaining Balance</span>
//             <strong>${selectedRow?.remainingBalance || "0.00"}</strong>
//           </div>
//           <button
//             type="button"
//             onClick={() => setActivePopup("voucherTotals")}
//             className="voucher-total-link"
//           >
//             <Calculator size={12} />
//             View Voucher Totals
//           </button>
...
//       {/* SECOND-SCREEN NAVIGATION — actual existing detail tabs */}
//       <div className="voucher-secondary-trigger">
//         <div className="flex items-center justify-between gap-4">
//           <div className="voucher-secondary-tabs">
//             {["Account Info", "Other Info", "Line Notes"].map((tab) => (
//               <button
//                 key={tab}
//                 type="button"
//                 onClick={() => {
...
//                   setIsChildFormView(true);
//                 }}
//                 className={`voucher-secondary-tab ${showVoucherDetail && activeChildTab === tab ? "active" : ""}`}
//               >
//                 {tab}
//               </button>
//             ))}
//           </div>
//           <div className="flex items-center gap-2 shrink-0">
//             <span className="mr-2 text-[10px] text-[#8a98ab] hidden md:inline">Last saved just now</span>
//             <button type="button" onClick={handleSave} className="voucher-primary-btn"><Save size={14} /> Save</button>
//             <button type="button" onClick={() => { handleSave(); handleAdd(); }} className="voucher-outline-btn"><Plus size={14} /> Save &amp; New</button>
//             <button type="button" onClick={handleDiscard} className="voucher-outline-btn">Cancel</button>
//           </div>
//         </div>
//       </div>

//       {showVoucherDetail && (
//         <div className="voucher-second-screen">
//           <div className="voucher-second-screen-closebar">
//             <button
//               type="button"
//               className="voucher-second-screen-close"
//               onClick={() => setShowVoucherDetail(false)}
//               title="Close voucher detail"
//             >
//               <X size={13} />
//               Close
//             </button>
...
//           ) : (
//             <div className="space-y-4">
//               {/* DETAIL CONTENT - selected by the real secondary tabs above */}
//               {/* TABS CONTAINER FOR DETAIL FORM */}
//               <div className="voucher-detail-content-shell">
//                 {!selectedChildRow ? (
//                   <div className="text-center text-gray-400 italic text-[11px] py-8">
//                     No line selected. Add or select a line to edit details.
//                   </div>
...
//         {/* BOTTOM LEFT CHILD ACTION MENU LINKS */}
//         <div className="voucher-nested-tabs mt-4 flex flex-wrap gap-2 px-2 pb-2">
//           {[
//             { name: "Vendor Labor" },
//             { name: "Currency Line" },
//             { name: "Customs Info" }
...
//                 type="button"
//                 onClick={() => setActiveNestedTabs(prev => ({ ...prev, [tab.name]: true }))}
//                 className={`voucher-nested-tab ${activeNestedTabs[tab.name] ? "active" : ""}`}
//               >
//                 {tab.name}
//               </button>
//             );
...
//       {/* Keep footer, secondary navigation, and second screen inside the same
//           horizontal container as the first screen so all outer edges align. */}
//       </div>
...
const FormInput = ({
  ...
  const actualReadOnly =
    readOnly || (isCurrentRecordApprovedLocked && !isApproveCheckbox);
  const actualDisabled =
    disabled ||
    (isCurrentRecordApprovedLocked &&
      !isApproveCheckbox &&
      type === "checkbox");
...
const masterColumns = [
  ...
  { id: "originalVoucher", key: "originalVoucher", label: "Original Voucher" },
  {
    id: "accountDescriptionAp",
    key: "accountDescriptionAp",
    label: "A/P Account",
  },
  {
    id: "accountDescriptionCash",
    key: "accountDescriptionCash",
    label: "Cash Account",
  },
  {
    id: "voucherLineRecalcMethod",
    key: "voucherLineRecalcMethod",
    label: "Recalc Method",
  },
  {
    id: "whenSalesTaxChanged",
    key: "whenSalesTaxChanged",
    label: "Sales Tax Method",
  },
  { id: "totalTax", key: "totalTax", label: "Total Tax" },
  {
    id: "remainingBalance",
  ...
  {
    id: "anticipatedPayDate",
    key: "anticipatedPayDate",
    label: "Anticipated Pay Date",
    type: "date",
  },
  { id: "expenseReportId", key: "expenseReportId", label: "Expense Report ID" },
  ...
  // Check columns
  {
    id: "checkCashAcctDesc",
    key: "checkCashAcctDesc",
    label: "Payment Cash Account Description",
  },
  { id: "checkNumber", key: "checkNumber", label: "Payment Number" },
  { id: "checkDate", key: "checkDate", label: "Payment Date", type: "date" },
  {
    id: "checkDiscountTaken",
    key: "checkDiscountTaken",
    label: "Payment Discount Taken",
  },
  { id: "checkAmount", key: "checkAmount", label: "Payment Amount" },
  {
    id: "checkPayTransType",
    key: "checkPayTransType",
  ...
  {
    id: "checkPostFiscalYear",
    key: "checkPostFiscalYear",
    label: "Payment Post Fiscal Year",
  },
  {
    id: "checkPostPeriod",
    key: "checkPostPeriod",
    label: "Payment Post Period",
  },
  {
    id: "checkPostSubperiod",
    key: "checkPostSubperiod",
    label: "Payment Post Subperiod",
  },

  // Recur columns
  { id: "recurCode", key: "recurCode", label: "Recurring Code" },
  {
    id: "recurStartFiscalYear",
    key: "recurStartFiscalYear",
    label: "Recurring Start Fiscal Year",
  },
  {
    id: "recurStartPeriod",
    key: "recurStartPeriod",
    label: "Recurring Start Period",
  },
  {
    id: "recurStartSubperiod",
    key: "recurStartSubperiod",
    label: "Recurring Start Subperiod",
  },
  {
    id: "recurStartEndingDate",
    key: "recurStartEndingDate",
    label: "Recurring Start Ending Date",
  },
  {
    id: "recurEndFiscalYear",
    key: "recurEndFiscalYear",
    label: "Recurring End Fiscal Year",
  },
  {
    id: "recurEndPeriod",
    key: "recurEndPeriod",
    label: "Recurring End Period",
  },
  {
    id: "recurEndSubperiod",
    key: "recurEndSubperiod",
    label: "Recurring End Subperiod",
  },
  {
    id: "recurEndEndingDate",
    key: "recurEndEndingDate",
    label: "Recurring End Ending Date",
  },
  {
    id: "recurLastVchrFiscalYear",
    key: "recurLastVchrFiscalYear",
    label: "Recurring Last Voucher Fiscal Year",
  },
  {
    id: "recurLastVchrPeriod",
    key: "recurLastVchrPeriod",
    label: "Recurring Last Voucher Period",
  },
  {
    id: "recurLastVchrSubperiod",
    key: "recurLastVchrSubperiod",
    label: "Recurring Last Voucher Subperiod",
  },

  // Subcontractor Info columns
  {
    id: "subcontractorInvoicePopDate",
    key: "subcontractorInvoicePopDate",
    label: "Invoice Period of Performance Date",
    type: "date",
  },
  {
    id: "subcontractorDeliveryValue",
    key: "subcontractorDeliveryValue",
    label: "Delivery Value",
  },
  {
    id: "subcontractorInvoiceType",
    key: "subcontractorInvoiceType",
    label: "Invoice Type",
  },

  // Notes/Doc Loc columns
  {
    id: "notesPrintOnCheck",
    key: "notesPrintOnCheck",
    label: "Print Notes on Check",
    type: "checkbox",
  },
  { id: "notesText", key: "notesText", label: "Blank Laser Check Notes" },
  {
    id: "notesDocLocation",
    key: "notesDocLocation",
    label: "Document Location",
  },

  // Entry Defaults columns
  {
    id: "defaultAllowPayVendorWarning",
    key: "defaultAllowPayVendorWarning",
    label: "Allow Pay Vendor on Warning Status",
    type: "checkbox",
  },
  {
    id: "defaultAddDiscDiffFirstLine",
    key: "defaultAddDiscDiffFirstLine",
    label: "Add Discount Difference to First Line",
    type: "checkbox",
  },
  {
    id: "defaultUseAccountDesc",
    key: "defaultUseAccountDesc",
    label: "Use Account Description",
    type: "checkbox",
  },
  {
    id: "defaultUseOwningOrg",
    key: "defaultUseOwningOrg",
    label: "Use Owning Org",
    type: "checkbox",
  },
  {
    id: "defaultAllowDuplicateInvNum",
    key: "defaultAllowDuplicateInvNum",
    label: "Allow Duplicate Invoice Numbers",
    type: "checkbox",
  },
  {
    id: "defaultSaveTeVoucher",
    key: "defaultSaveTeVoucher",
    label: "Save as TE Voucher",
    type: "checkbox",
  },
];
...
const taxVatOptions = [
  ...
  {
    value: "MD0",
    label: "MD0 - MARYLAND Zero Tax for Exempt (0.00%)",
    rate: "0.00",
  },
  { value: "VA", label: "VA - VIRGINIA (6.00%)", rate: "6.00" },
];
...
const calculateRowUpdates = (item) => {
  ...
  const taxability = String(item.taxability || "")
    .trim()
    .toUpperCase();
  ...
  // Determine labor amount (sync with costAmount unless explicitly overridden with a valid non-zero amount)
  const currentLaborAmt = item.laborAmountExplicit
    ? toNumber(item.laborAmount, cost)
    : cost;
  const finalLaborAmount =
    currentLaborAmt > 0 ? currentLaborAmt.toFixed(2) : cost.toFixed(2);
...
const updateHeaderBalances = (record) => {
  const invAmt = toNumber(record.invoiceAmount || 0);
  const sumTax = (record.detailLines || []).reduce(
    (sum, line) =>
      sum +
      toNumber(line.salesVatTaxAmt || 0) +
      toNumber(line.useReverseTaxAmt || 0),
    0,
  );
  const discAmt = toNumber(record.discountAmount || 0);
  const paidAmt = toNumber(record.checkAmount || 0);
  ...
  // Calculate sum of totalAmt of all detail lines
  const computedDueAmt = (record.detailLines || []).reduce(
    (sum, line) => sum + toNumber(line.totalAmt || 0),
    0,
  );
  const finalDueAmt =
    record.detailLines && record.detailLines.length > 0
      ? computedDueAmt
      : invAmt + sumTax - discAmt;
...
const normalizeAccountOptions = (items = []) =>
  items
    .map((item) => {
      const value =
        item.acctId ??
...
const normalizeOrganizationOptions = (items = []) =>
  items
    .map((item) => {
      const value =
        item.orgId ??
...
const mapHeaderToRecord = (header, lines = []) => {
  ...
  const record = {
    ...
    defaultAllowDuplicateInvNum: "N",
    defaultSaveTeVoucher: "N",
    detailLines: lines.map((l, idx) =>
      mapLineToDetailLine(l, idx, invoiceAmount, header.disc_pct_rt ?? 0),
    ),
    isDirty: false,
  };
  return updateHeaderBalances(record);
...
const mapLineToDetailLine = (
  lineAggregate,
  index,
  parentInvoiceAmount = 0,
  parentDiscountPercent = 0,
) => {
  const line = lineAggregate.line || lineAggregate;
  const account = (lineAggregate.accounts || [])[0] || {};
  const labVendor = (lineAggregate.labVendors || [])[0] || {};
  const costAmount = toNumber(
    account.cst_amt ?? line.ext_cst_amt ?? line.net_amt ?? 0,
  );
  const salesTaxAmount = toNumber(
    account.sales_tax_amt ?? line.sales_tax_amt ?? 0,
  );
  const totalBeforeDiscount = toNumber(
    account.tot_bef_disc_amt ?? line.tot_bef_disc_amt ?? line.ext_cst_amt ?? 0,
  );
  ...
  if (rawPct > 0) {
    ...
    percentStr = ((costAmount / toNumber(parentInvoiceAmount)) * 100).toFixed(
      2,
    );
  }
  ...
  // Derive tax rate
  let derivedTaxRate = toNumber(
    line.sales_tax_pct_rt ?? line.tax_pct_rt ?? line.tax_rate ?? 0,
  );
  if (derivedTaxRate === 0 && costAmount > 0 && salesTaxAmount > 0) {
    derivedTaxRate = (salesTaxAmount / costAmount) * 100;
  } else if (derivedTaxRate === 0 && line.sales_tax_cd) {
  ...
  // Handle recovery rate conversion (e.g., DB ratio 0.8 -> UI 80.00)
  const rawRecRate = toNumber(line.recovery_rt ?? account.recovery_rt ?? 100);
  const recoveryRateStr =
    rawRecRate > 0 && rawRecRate <= 1.0
      ? (rawRecRate * 100).toFixed(2)
      : rawRecRate.toFixed(2);
  const calculatedRecoveryAmt =
    (salesTaxAmount * toNumber(recoveryRateStr)) / 100;
  ...
  return {
    ...
    account: account.account_id ?? "",
    accountName:
      account.acct_name ?? account.acctName ?? account.accountName ?? "",
    organization: account.org_id ?? "",
    organizationName:
      account.org_name ?? account.orgName ?? account.organizationName ?? "",
    project: account.project_id ?? "",
    projectName: account.proj_name ?? account.projectName ?? "",
    projAcctAbbrev: account.project_account_abbrv_cd ?? "",
    costAmount: costAmount.toFixed(2),
    ...
    taxability:
      ...
      line.taxable_fl === "S"
        ? "T"
        : line.taxable_fl === "R" || line.taxable_fl === "U"
          ? "R"
          : "N",
    taxVatCode: line.sales_tax_cd ?? "",
    taxRate: derivedTaxRate.toFixed(2),
    ...
    dateOfSupply: line.vat_supply_dt
      ...
      : "",
    unitOfMeasure: line.um_cd ?? "",
    subLine: labVendor.sub_ln_no
      ? String(labVendor.sub_ln_no)
      : String(line.voucher_ln_no ?? index + 1),
    vendorEmployee: labVendor.vend_empl_id ?? "",
    glc: labVendor.genl_lab_cat_cd ?? "",
    plc: labVendor.bill_lab_cat_cd ?? "",
    hours: labVendor.vendor_hrs ? String(labVendor.vendor_hrs) : "",
    laborAmount: labVendor.vendor_amt
      ? String(labVendor.vendor_amt)
      : costAmount.toFixed(2),
    vatRecoveryAmt: labVendor.recovery_amt
      ? String(labVendor.recovery_amt)
      : calculatedRecoveryAmt.toFixed(2),
...
const buildVoucherPayload = (record) => {
  ...
  return {
    header: compactDbObject({
      vendor_id: record.vendor || "VND-1001",
      s_voucher_type:
        record.voucherType === "AP" || record.voucherType === "AP Voucher"
          ? "AP"
          : record.voucherType || "AP",
      terms_dc: normalizeTerms(record.terms),
      approved_fl: record.approved || "N",
      posted_ap_fl: "N",
      invc_id: record.invoiceNumber || `INV-${Date.now()}`.slice(0, 50),
...
const PopupShell = ({ title, children, onClose, wide = false }) => (
  <div className="fixed inset-0 z-[9999] bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
    <div
      className={`bg-white border border-slate-200/80 shadow-2xl rounded-2xl ${wide ? "w-[78vw]" : "w-[54vw]"} max-h-[86vh] overflow-y-auto overflow-hidden`}
    >
      <div className="flex items-center justify-between bg-slate-50 border-b border-slate-100 px-4 py-3 select-none">
...
const isBlankDetailLine = (line) => {
  ...
  const hasTextValue = meaningfulFields.some((field) =>
...
const ManageAccountsPayableVouchers = () => {
  ...
  const [accountDropdownOptions, setAccountDropdownOptions] =
    useState(accountOptions);
  const [organizationDropdownOptions, setOrganizationDropdownOptions] =
    useState(organizationOptions);
  const [projectDropdownOptions, setProjectDropdownOptions] =
  ...
  useEffect(() => {
    const fetchVendorLaborDropdowns = async () => {
      try {
        const empRes =
          (await api
            .get("/api/accounts-payable-vouchers/vendor-employees")
            .catch(() => null)) ||
          (await api.get("/api/vendor-employees").catch(() => null));
        if (empRes && Array.isArray(empRes.data)) {
          const formattedEmps = empRes.data
            .map((emp) => {
              const val =
                emp.vend_empl_id ??
                emp.vendEmplId ??
        ...
        const plcRes =
          (await api
            .get("/api/accounts-payable-vouchers/plc-codes")
            .catch(() => null)) ||
          (await api.get("/api/plc-codes").catch(() => null));
        if (plcRes && Array.isArray(plcRes.data)) {
          const formattedPlcs = plcRes.data
            .map((p) => {
              const val =
                p.bill_lab_cat_cd ??
                p.billLabCatCd ??
        ...
        const glcRes =
          (await api
            .get("/api/accounts-payable-vouchers/glc-codes")
            .catch(() => null)) ||
          (await api.get("/api/glc-codes").catch(() => null));
        if (glcRes && Array.isArray(glcRes.data)) {
          const formattedGlcs = glcRes.data
            .map((g) => {
              const val =
                g.genl_lab_cat_cd ??
                g.genlLabCatCd ??
  ...
  const [records, setRecords] = useState(() =>
    initialMockRecords.map(updateHeaderBalances),
  );
  ...
  const selectedChildRow =
    (selectedRow?.detailLines || []).find(
      (line) =>
        getRowKey(line) ===
        (selectedChildRowState ? getRowKey(selectedChildRowState) : ""),
    ) || selectedChildRowState;
  ...
  const selectRecord = async (record) => {
    ...
    try {
      const response = await api.get(`${apiBaseUrl}/${record.voucherKey}`);
      const hydrated = mapHeaderToRecord(
        response.data.header || {},
        response.data.lines || [],
      );
      setRecords((prev) =>
        prev.map((item) =>
          getRowKey(item) === getRowKey(record) ? hydrated : item,
        ),
      );
      setSelectedRow(hydrated);
      setSelectedIds(new Set([getRowKey(hydrated)]));
      ...
      setSelectedChildRow(hydratedFirstLine);
      setSelectedChildIds(
        hydratedFirstLine ? new Set([getRowKey(hydratedFirstLine)]) : new Set(),
      );
    } catch (error) {
      toast.error(
        error.response?.data?.detail || "Unable to load voucher details.",
      );
    }
  };
  ...
  const loadVouchers = async () => {
    ...
    try {
      const response = await api.get(`${apiBaseUrl}?limit=100`);
      const nextRecords = (response.data || []).map((header) =>
        mapHeaderToRecord(header, []),
      );
      setRecords(nextRecords);
      if (nextRecords.length > 0) {
        await selectRecord(nextRecords[0]);
      ...
      toast.error(
        error.response?.data?.detail ||
          "Unable to load accounts payable vouchers.",
      );
    } finally {
      setIsLoading(false);
    }
  ...
  const [taxVatDropdownOptions, setTaxVatDropdownOptions] =
    useState(taxVatOptions);
  ...
  const loadDetailReferenceData = async () => {
    ...
    try {
      ...
      if (organizationsResponse.status === "fulfilled") {
        const options = normalizeOrganizationOptions(
      ...
      if (projectsResponse.status === "fulfilled") {
        setProjectDropdownOptions(
          normalizeProjectOptions(projectsResponse.value || []),
        );
      }

      if (
        salesTaxesRes.status === "fulfilled" &&
        Array.isArray(salesTaxesRes.value?.data)
      ) {
        const liveTaxes = salesTaxesRes.value.data
          .map((t) => {
            const code = t.taxCode || t.tax_code || "";
            const desc = t.description || t.desc || "";
            const rate = t.compositeTaxRate ?? t.taxRate ?? "0.00";
  ...
  const isApprovedLocked = (record) =>
    record?.approved === "Y" && !record?.isDirty;

  // Master Fields Change
  const handleFieldChange = (rowId, field, value) => {
    if (
      field !== "approved" &&
      isApprovedLocked(selectedRow) &&
      getRowKey(selectedRow) === String(rowId)
    ) {
      toast.warn("Approved vouchers cannot be edited.");
      return;
    }
      ...
      prev.map((r) => {
        if (getRowKey(r) === String(rowId)) {
          ...
          if (field === "invoiceAmount") {
            ...
            const discPct = toNumber(value || 0);
            updatedRecord.discountAmount = ((invAmt * discPct) / 100).toFixed(
              2,
            );
            updatedRecord.detailLines = (r.detailLines || []).map((line) => {
              const cost = toNumber(line.costAmount || 0);
              const updatedLine = {
                ...line,
                discount: ((cost * discPct) / 100).toFixed(2),
              };
              Object.assign(updatedLine, calculateRowUpdates(updatedLine));
              return updatedLine;
            });
          ...
          const sumTax = (updatedRecord.detailLines || []).reduce(
            (sum, line) => sum + toNumber(line.salesVatTaxAmt),
            0,
          );
          updatedRecord.totalTax = sumTax.toFixed(2);
          return updateHeaderBalances(updatedRecord);
  ...
  const handleRecalculateLines = (method) => {
    ...
    const updatedLines = (selectedRow.detailLines || []).map((line) => {
      ...
      if (method === "Recalculate Cost") {
        ...
        const cost = toNumber(line.costAmount || 0);
        updatedLine.percent =
          invAmt > 0 ? ((cost / invAmt) * 100).toFixed(2) : "100.00";
      }

      // Sync line discount
      updatedLine.discount = (
        (toNumber(updatedLine.costAmount) * discPct) /
        100
      ).toFixed(2);
    ...
    const sumTax = updatedLines.reduce(
      (sum, line) => sum + toNumber(line.salesVatTaxAmt),
      0,
    );
    ...
    // Refresh selected child row if active
    if (selectedChildRow) {
      const refreshed = updatedLines.find(
        (line) => getRowKey(line) === getRowKey(selectedChildRow),
      );
      if (refreshed) {
        setSelectedChildRow(refreshed);
      }
  ...
  const handleRecalculateTaxMethod = (method) => {
    ...
    const updatedLines = (selectedRow.detailLines || []).map((line) => {
      ...
      if (method === "Recalculate Tot Before Disc") {
        updatedLine.totBeforeDisc = (cost + tax).toFixed(2);
        updatedLine.totalAmt = (
          toNumber(updatedLine.totBeforeDisc) - toNumber(line.discount || 0)
        ).toFixed(2);
      } else if (method === "Recalculate Discount") {
        updatedLine.discount = (((cost + tax) * discPct) / 100).toFixed(2);
        updatedLine.totalAmt = (
...
const handleVoucherNavigation = (direction) => {
  ...
  // Don't go outside the available records
  if (newIndex < 0 || newIndex >= records.length) return;
  ...
  const handleDelete = async () => {
    ...
    const idsToDelete =
      selectedIds.size > 0 ? selectedIds : new Set([getRowKey(selectedRow)]);
    const recordsToDelete = records.filter((r) =>
      idsToDelete.has(getRowKey(r)),
    );
  ...
  const buildApprovalQueueVoucher = (record) => ({
    ...
    cashAccountsDescription:
      record.checkCashAcctDesc || record.accountDescriptionCash || "",
    fiscalYear: record.fiscalYear || "",
    period: record.period || "",
    subperiod: record.subperiod || "",
    poDiscrepancy: "N",
  ...
  const handleSave = async () => {
    ...
    if (
      !selectedRow.fiscalYear?.trim() ||
      !String(selectedRow.period || "").trim() ||
      !selectedRow.vendor?.trim()
    ) {
      return toast.error(
        "Fiscal Year, Period, and Vendor are required fields!",
      );
    }

    // Costpoint AP Voucher Validations:
    const computedCostAmount = (selectedRow.detailLines || []).reduce(
      (sum, line) => sum + toNumber(line.costAmount || 0),
      0,
    );
    const headerInvoiceAmount = toNumber(selectedRow.invoiceAmount || 0);
    if (Math.abs(computedCostAmount - headerInvoiceAmount) > 0.01) {
      return toast.error(
        `Total of line Cost Amounts ($${computedCostAmount.toFixed(2)}) must equal the Invoice Amount ($${headerInvoiceAmount.toFixed(2)})!`,
      );
    }

    const computedSalesTax = (selectedRow.detailLines || []).reduce(
      (sum, line) => sum + toNumber(line.salesVatTaxAmt || 0),
      0,
    );
    const headerTotalTax = toNumber(selectedRow.totalTax || 0);
    if (Math.abs(computedSalesTax - headerTotalTax) > 0.01) {
      return toast.error(
        `Total of line Sales/VAT Tax ($${computedSalesTax.toFixed(2)}) must equal the Header Total Tax ($${headerTotalTax.toFixed(2)})!`,
      );
    }
    ...
    try {
      const response = selectedRow.voucherKey
        ? await api.put(
            `${apiBaseUrl}/${selectedRow.voucherKey}`,
            buildVoucherPayload(selectedRow),
          )
        : await api.post(apiBaseUrl, buildVoucherPayload(selectedRow));
      const voucherKey = response.data?.voucherKey || selectedRow.voucherKey;
      ...
      if (voucherKey) {
        const detailResponse = await api.get(`${apiBaseUrl}/${voucherKey}`);
        const savedRecord = mapHeaderToRecord(
          detailResponse.data.header || {},
          detailResponse.data.lines || [],
        );
        setRecords((prev) =>
          prev.map((r) =>
            getRowKey(r) === getRowKey(selectedRow) ? savedRecord : r,
          ),
        );
        await selectRecord(savedRecord);
        ...
        if (savedRecord.approved !== "Y") {
          sessionStorage.setItem(
            ...
            JSON.stringify(buildApprovalQueueVoucher(savedRecord)),
          );
          toast.success(
            response.data?.message || "Voucher saved for approval.",
          );
          return;
        }
      } else {
        ...
        if (selectedRow.approved !== "Y") {
          sessionStorage.setItem(
            "apVoucherPendingApproval",
            JSON.stringify(buildApprovalQueueVoucher(selectedRow)),
          );
          toast.success(
            response.data?.message || "Voucher saved for approval.",
          );
    ...
    setRecords((prev) =>
      prev.map((r) => {
        if (getRowKey(r) === parentId) {
          ...
          const sumTax = updatedChildren.reduce(
            (sum, line) => sum + toNumber(line.salesVatTaxAmt),
            0,
          );
          let updatedRecord = {
            ...r,
            detailLines: updatedChildren,
            totalTax: sumTax.toFixed(2),
            isDirty: true,
          };
          return updateHeaderBalances(updatedRecord);
  ...
  const handleChildAdd = () => {
    ...
    const existingBlankLine = (selectedRow.detailLines || []).find(
      (line) => line.tempId && isBlankDetailLine(line),
    );
    if (existingBlankLine) {
      setSelectedChildIds(new Set([getRowKey(existingBlankLine)]));
      setSelectedChildRow(existingBlankLine);
    ...
    setRecords((prev) =>
      prev.map((r) => {
        if (getRowKey(r) === parentId) {
          return {
            ...r,
            detailLines: [...(r.detailLines || []), newChild],
            isDirty: true,
          };
        }
        return r;
    ...
    setSelectedRow((prev) => {
      ...
      return {
        ...prev,
        detailLines: [...(prev.detailLines || []), newChild],
        isDirty: true,
      };
    });
  ...
  const handleChildDelete = () => {
    ...
    const idsToDelete =
      selectedChildIds.size > 0
        ? selectedChildIds
        : new Set([getRowKey(selectedChildRow)]);

    setRecords((prev) =>
      prev.map((r) => {
        if (getRowKey(r) === parentId) {
          const remaining = (r.detailLines || []).filter(
            (c) => !idsToDelete.has(getRowKey(c)),
          );
          const normalized = remaining.map((c, idx) => ({
            ...c,
            lineNo: String(idx + 1),
          }));
          return { ...r, detailLines: normalized, isDirty: true };
    ...
    setSelectedRow((prev) => {
      if (!prev || getRowKey(prev) !== parentId) return prev;
      const remaining = (prev.detailLines || []).filter(
        (c) => !idsToDelete.has(getRowKey(c)),
      );
      const normalized = remaining.map((c, idx) => ({
        ...c,
  ...
  const handleChildDiscard = async () => {
    ...
    const cleanLines = (selectedRow.detailLines || []).map((line) => ({
      ...line,
      isDirty: false,
    }));
    const cleanRow = { ...selectedRow, detailLines: cleanLines };
    setRecords((prev) =>
      prev.map((r) => (getRowKey(r) === getRowKey(selectedRow) ? cleanRow : r)),
    );
    setSelectedRow(cleanRow);
    setSelectedChildRow((prev) => {
      if (!prev) return prev;
      const refreshed = cleanLines.find(
        (line) => getRowKey(line) === getRowKey(prev),
      );
      return refreshed || prev;
    });
    toast.info("Unsaved line changes discarded.");
    ...
    if (column.key === "taxability") {
      return {
        ...
        onBlur: (val) => {
          ...
          if (clean && !["T", "R", "N"].includes(clean)) {
            toast.warn(
              "Invalid taxability code detected. Please enter a valid classification: 'T' for Sales/VAT, 'R' for Use/Reverse tax, or 'N' for Non-taxable.",
              {
                toastId: "invalid-taxability",
              },
            );
  ...
  const renderVoucherTotalsPopup = () => {
    ...
    const computedCostAmount = lines.reduce(
      (sum, line) => sum + toNumber(line.costAmount, 0),
      0,
    );
    const computedSalesTaxAmount = lines.reduce(
      (sum, line) => sum + toNumber(line.salesVatTaxAmt, 0),
      0,
    );
    const computedRecoveryAmount = lines.reduce(
      (sum, line) => sum + toNumber(line.recoveryAmt, 0),
      0,
    );
    const computedTotalBeforeDiscount = lines.reduce(
      (sum, line) => sum + toNumber(line.totBeforeDisc, 0),
      0,
    );
    const computedDiscountAmount = lines.reduce(
      (sum, line) => sum + toNumber(line.discount, 0),
      0,
    );
    const computedTotalAmount = lines.reduce(
      (sum, line) => sum + toNumber(line.totalAmt, 0),
      0,
    );
    const computedUseTaxAmount = lines.reduce(
      (sum, line) => sum + toNumber(line.useReverseTaxAmt, 0),
      0,
    );

    const costAmount =
      selectedRow.invoiceAmount !== undefined &&
      selectedRow.invoiceAmount !== null
        ? selectedRow.invoiceAmount
        : computedCostAmount.toFixed(2);

    const salesTaxAmount =
      selectedRow.totalTax !== undefined && selectedRow.totalTax !== null
        ? selectedRow.totalTax
        : computedSalesTaxAmount.toFixed(2);

    const recoveryAmount =
      selectedRow.recoveryAmount !== undefined &&
      selectedRow.recoveryAmount !== null
        ? selectedRow.recoveryAmount
        : computedRecoveryAmount.toFixed(2);

    const totalBeforeDiscount =
      selectedRow.totalBeforeDiscount !== undefined &&
      selectedRow.totalBeforeDiscount !== null
        ? selectedRow.totalBeforeDiscount
        : computedTotalBeforeDiscount.toFixed(2);

    const discountAmount =
      selectedRow.discountAmount !== undefined &&
      selectedRow.discountAmount !== null
        ? selectedRow.discountAmount
        : computedDiscountAmount.toFixed(2);

    const totalAmount =
      selectedRow.totalAmount !== undefined && selectedRow.totalAmount !== null
        ? selectedRow.totalAmount
        : computedTotalAmount.toFixed(2);

    const amountPaid =
      selectedRow.checkAmount !== undefined && selectedRow.checkAmount !== null
        ? selectedRow.checkAmount
        : toNumber(selectedRow.checkAmount, 0).toFixed(2);

    const discountTakenAmount =
      selectedRow.checkDiscountTaken !== undefined &&
      selectedRow.checkDiscountTaken !== null
        ? selectedRow.checkDiscountTaken
        : toNumber(selectedRow.checkDiscountTaken, 0).toFixed(2);

    const useTaxAmount =
      selectedRow.useTaxAmount !== undefined &&
      selectedRow.useTaxAmount !== null
        ? selectedRow.useTaxAmount
        : computedUseTaxAmount.toFixed(2);
        ...
        <FormSection title="Voucher Totals">
          <div className="max-w-md space-y-2 py-4">
            <FormInput
              ...
              value={costAmount}
              onChange={(e) =>
                handleFieldChange(activeId, "invoiceAmount", e.target.value)
              }
              inputClassName="text-right"
            />
            <FormInput
              ...
              value={salesTaxAmount}
              onChange={(e) =>
                handleFieldChange(activeId, "totalTax", e.target.value)
              }
              inputClassName="text-right"
            />
            <FormInput
              ...
              value={recoveryAmount}
              onChange={(e) =>
                handleFieldChange(activeId, "recoveryAmount", e.target.value)
              }
              inputClassName="text-right"
            />
            <FormInput
            ...
            <FormInput
              ...
              value={discountAmount}
              onChange={(e) =>
                handleFieldChange(activeId, "discountAmount", e.target.value)
              }
              inputClassName="text-right"
            />
            <FormInput
              ...
              value={totalAmount}
              onChange={(e) =>
                handleFieldChange(activeId, "totalAmount", e.target.value)
              }
              inputClassName="text-right"
            />
            <FormInput
              ...
              value={amountPaid}
              onChange={(e) =>
                handleFieldChange(activeId, "checkAmount", e.target.value)
              }
              inputClassName="text-right"
            />
            <FormInput
            ...
            <FormInput
              ...
              value={useTaxAmount}
              onChange={(e) =>
                handleFieldChange(activeId, "useTaxAmount", e.target.value)
              }
              inputClassName="text-right"
            />
          </div>
  ...
  const renderVendorLaborContainer = () => {
    ...
    const vendorLaborColumns = [
      { id: "subLine", key: "subLine", label: "Line Number", required: true },
      {
        id: "vendorEmployee",
        key: "vendorEmployee",
        label: "Vendor Employee",
      ...
      { id: "hours", key: "hours", label: "Labor Hours", required: true, type: "number" },
      {
        id: "laborAmount",
        key: "laborAmount",
        label: "Labor Cost Amount",
      ...
      {
        id: "effectiveBillingDate",
        key: "effectiveBillingDate",
        label: "Billing Effective Date",
        required: true,
        type: "date",
      },
    ...
    const checkedLines =
      selectedChildIds.size > 0
        ? (selectedRow?.detailLines || []).filter((line) =>
            selectedChildIds.has(getRowKey(line)),
          )
        : selectedChildRow
          ? [selectedChildRow]
          : selectedRow?.detailLines || [];

    const linesToRender = checkedLines;
    const data = linesToRender.map((line) => {
      const lineCost =
        line.costAmount !== undefined && line.costAmount !== ""
          ? String(line.costAmount)
          : "0.00";
      const lineRecovery =
        line.recoveryAmt !== undefined && line.recoveryAmt !== ""
          ? String(line.recoveryAmt)
          : "0.00";
      return {
        ...line,
        subLine: line.subLine || line.lineNo || "",
    ...
    const activeChildLine =
      selectedChildRow &&
      linesToRender.some((l) => getRowKey(l) === getRowKey(selectedChildRow))
        ? selectedChildRow
        : linesToRender[0] || null;

    const activeCost =
      activeChildLine?.costAmount !== undefined &&
      activeChildLine?.costAmount !== ""
        ? String(activeChildLine.costAmount)
        : "0.00";
    const activeRecovery =
      activeChildLine?.recoveryAmt !== undefined &&
      activeChildLine?.recoveryAmt !== ""
        ? String(activeChildLine.recoveryAmt)
        : "0.00";
    ...
    return (
      ...
      >
        <Toolbar
          ...
          buttonsDisable={["paste", "discard", "save"]}
          currentIndex={linesToRender.findIndex(
            (d) => getRowKey(d) === getRowKey(activeChildLine),
          )}
          totalRecords={linesToRender.length}
          handleNavigate={(dir) => {
            const idx = linesToRender.findIndex(
              (d) => getRowKey(d) === getRowKey(activeChildLine),
            );
            let targetChild = null;
            if (dir === "start" && linesToRender.length > 0)
              targetChild = linesToRender[0];
            else if (dir === "prev" && idx > 0)
              targetChild = linesToRender[idx - 1];
            else if (dir === "next" && idx < linesToRender.length - 1)
              targetChild = linesToRender[idx + 1];
            else if (dir === "end" && linesToRender.length > 0)
              targetChild = linesToRender[linesToRender.length - 1];
            if (targetChild) {
              setSelectedChildRow(targetChild);
            }
          }}
        ...
        {!activeChildLine ? (
          <div className="text-center text-gray-400 italic text-[11px] py-8">
            No line selected. Select line(s) in A/P Voucher Detail to view
            vendor labor.
          </div>
        ) : !isVendorLaborFormView ? (
          <div className="p-2">
            <ReusableTable
              ...
              onSelectAll={(e) => {
                if (e.target.checked) {
                  setSelectedChildIds(
                    new Set((selectedRow?.detailLines || []).map(getRowKey)),
                  );
                } else {
                  setSelectedChildIds(new Set());
                }
              ...
              onFieldChange={(rowId, field, val) =>
                handleChildFieldChange(rowId, field, val)
              }
              rowKey={getRowKey}
              maxHeight="max-h-64"
            />
          ...
          <FormSection title="Vendor Labor">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-2">
              ...
              <FormInput
                ...
                type="number"
                value={
                  activeChildLine.laborAmountExplicit
                    ? activeChildLine.laborAmount
                    : activeCost
                }
                onChange={(e) => {
                  handleChildFieldChange(
  ...
  const renderCustomsInfoContainer = () => {
    ...
    const customsColumns = [
      ...
      { id: "permitLicense", key: "permitLicense", label: "Permit/License" },
      {
        id: "intrastatReference",
        key: "intrastatReference",
        label: "Intrastat Reference",
      },
      { id: "deliveryTerms", key: "deliveryTerms", label: "Delivery Terms" },
      {
      ...
      {
        id: "declarationPoint",
        key: "declarationPoint",
        label: "Declaration Point",
      },
      { id: "lineNo", key: "lineNo", label: "Line" },
      {
        id: "shipFromEcCountry",
        key: "shipFromEcCountry",
        label: "Ship from EC Country",
      },
      { id: "mass", key: "mass", label: "Mass" },
      { id: "unitOfMeasure", key: "unitOfMeasure", label: "Unit of Measure" },
      {
      ...
      { id: "commodity", key: "commodity", label: "Commodity" },
      {
        id: "supplementalUnits",
        key: "supplementalUnits",
        label: "Supplemental Units",
      },
      { id: "correction", key: "correction", label: "Correction" },
      { id: "customsValue", key: "customsValue", label: "Value" },
      { id: "goodsServices", key: "goodsServices", label: "Goods/Services" },
          ...
          <div className="space-y-3">
            <FormSection title="Value Added Tax Information">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4 py-2">
                <FormInput
                  ...
                  value={selectedRow?.taxId || ""}
                  onChange={(e) =>
                    handleFieldChange(activeRecordId, "taxId", e.target.value)
                  }
                />
                <FormInput
                  label="Tax Date"
                  type="date"
                  value={selectedRow?.taxingDate || ""}
                  onChange={(e) =>
                    handleFieldChange(
                      activeRecordId,
                      "taxingDate",
            ...
            <FormSection title="Customs Header Information">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-2">
                <FormInput
                  label="Destination Country"
                  value={selectedRow?.destinationCountry || ""}
                  onChange={(e) =>
                    handleFieldChange(
                      activeRecordId,
                      "destinationCountry",
                ...
                <FormInput
                  label="Traffic Direction"
                  value={selectedRow?.trafficDirection || ""}
                  onChange={(e) =>
                    handleFieldChange(
                      activeRecordId,
                      "trafficDirection",
                ...
                <FormInput
                  ...
                  value={selectedRow?.permitLicense || ""}
                  onChange={(e) =>
                    handleFieldChange(
                      activeRecordId,
                      "permitLicense",
                ...
                <FormInput
                  label="Intrastat Reference"
                  value={selectedRow?.intrastatReference || ""}
                  onChange={(e) =>
                    handleFieldChange(
                      activeRecordId,
                      "intrastatReference",
                ...
                <FormInput
                  ...
                  value={selectedRow?.deliveryTerms || ""}
                  onChange={(e) =>
                    handleFieldChange(
                      activeRecordId,
                      "deliveryTerms",
                ...
                <FormInput
                  label="Mode of Transportation"
                  value={selectedRow?.modeOfTransportation || ""}
                  onChange={(e) =>
                    handleFieldChange(
                      activeRecordId,
                ...
                <FormInput
                  label="Declaration Point"
                  value={selectedRow?.declarationPoint || ""}
                  onChange={(e) =>
                    handleFieldChange(
                      activeRecordId,
                      "declarationPoint",
            ...
            <FormSection title="Customs Line Information">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-2">
                ...
                <FormInput
                  label="Ship from EC Country"
                  value={selectedChildRow?.shipFromEcCountry || ""}
                  onChange={(e) =>
                    handleChildFieldChange(
                      getRowKey(selectedChildRow),
                      "shipFromEcCountry",
                ...
                <FormInput
                  ...
                  value={selectedChildRow?.unitOfMeasure || ""}
                  onChange={(e) =>
                    handleChildFieldChange(
                      getRowKey(selectedChildRow),
                      "unitOfMeasure",
                ...
                <FormInput
                  label="Country of Origin"
                  value={selectedChildRow?.countryOfOrigin || ""}
                  onChange={(e) =>
                    handleChildFieldChange(
                      getRowKey(selectedChildRow),
                      "countryOfOrigin",
                ...
                <FormInput
                  label="Commodity"
                  value={selectedChildRow?.commodity || ""}
                  onChange={(e) =>
                    handleChildFieldChange(
                      getRowKey(selectedChildRow),
                      "commodity",
                ...
                <FormInput
                  label="Supplemental Units"
                  value={selectedChildRow?.supplementalUnits || ""}
                  onChange={(e) =>
                    handleChildFieldChange(
                      getRowKey(selectedChildRow),
                      "supplementalUnits",
                ...
                <FormInput
                  label="Correction"
                  value={selectedChildRow?.correction || ""}
                  onChange={(e) =>
                    handleChildFieldChange(
                      getRowKey(selectedChildRow),
                      "correction",
                ...
                <FormInput
                  label="Value"
                  value={selectedChildRow?.customsValue || ""}
                  onChange={(e) =>
                    handleChildFieldChange(
                      getRowKey(selectedChildRow),
                      "customsValue",
  ...
  const jumpToCode = (code) => {
    ...
    const found = records.find(
      (r) => String(r.voucher).toLowerCase() === String(code).toLowerCase(),
    );
    if (found) {
      selectRecord(found);
      setIsFormView(true);
  ...
  isCurrentRecordApprovedLocked =
    selectedRow?.approved === "Y" && !selectedRow?.isDirty;
      ...
      <div className="px-4 pb-4 pt-3 lg:px-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-x-5 gap-y-3">
                  ...
                  <div className="space-y-2">
                    <FormSearchSelect
                      ...
                      displayKey="label"
                      onSelect={(opt) =>
                        handleFieldChange(activeRecordId, "period", opt.value)
                      }
                    />
                    <FormSearchSelect
                      label="Terms"
                      ...
                      displayKey="label"
                      onSelect={(opt) =>
                        handleFieldChange(activeRecordId, "terms", opt.value)
                      }
                    />
                  </div>
                      ...
                      <div className="flex flex-col gap-3">
                        <FormSection title="Subcontractor Information">
                          <FormInput
                            ...
                            type="date"
                            value={
                              selectedRow?.subcontractorInvoicePopDate || ""
                            }
                            onChange={(e) =>
                              handleFieldChange(
                                activeRecordId,
                          ...
                          <FormInput
                            label="Delivery Value"
                            value={
                              selectedRow?.subcontractorDeliveryValue || ""
                            }
                            onChange={(e) =>
                              handleFieldChange(
                                activeRecordId,
                          ...
                          <div className="flex flex-col gap-1 w-full">
                            ...
                            <select
                              className="w-full px-2 py-0.5 rounded border text-[11px] font-medium bg-white border-slate-200 text-slate-800 hover:border-slate-300 focus:border-slate-400 focus:ring-1 focus:ring-slate-400 transition-all outline-none"
                              value={
                                selectedRow?.subcontractorInvoiceType || "None"
                              }
                              onChange={(e) =>
                                handleFieldChange(
                                  activeRecordId,
                                  "subcontractorInvoiceType",
                                  e.target.value,
                                )
                              }
                            >
                      ...
                      <div className="space-y-4">
                            ...
                            <div className="mt-3 pt-3 space-y-3">
                              ...
                              <FormSearchSelect
                                ...
                                options={[
                                  {
                                    value: "REC-01",
                                    label: "REC-01 - Monthly Operations",
                                  },
                                ]}
                                displayKey="label"
                                onSelect={(opt) =>
                              ...
                              <div className="space-y-1.5 pt-1.5">
                                ...
                                <div className="grid grid-cols-5 gap-2 items-center">
                                  ...
                                  <input
                                    ...
                                    className="w-full px-1.5 py-0.5 border border-slate-200 outline-none rounded text-[10px] font-medium text-slate-800 bg-white"
                                    value={
                                      selectedRow?.recurStartFiscalYear || ""
                                    }
                                    onChange={(e) =>
                                      handleFieldChange(
                                        activeRecordId,
                                  ...
                                  <input
                                    ...
                                    className="w-full px-1.5 py-0.5 border border-slate-200 outline-none rounded text-[10px] font-medium text-slate-800 bg-white"
                                    value={
                                      selectedRow?.recurStartSubperiod || ""
                                    }
                                    onChange={(e) =>
                                      handleFieldChange(
                                        activeRecordId,
                                  ...
                                  <input
                                    ...
                                    className="w-full px-1.5 py-0.5 border border-slate-200 outline-none rounded text-[10px] font-medium text-slate-400 bg-slate-50 cursor-not-allowed"
                                    value={
                                      selectedRow?.recurStartEndingDate || ""
                                    }
                                    disabled
                                  />
                                </div>
                                ...
                                <div className="grid grid-cols-5 gap-2 items-center pt-1">
                                  ...
                                  <input
                                    ...
                                    className="w-full px-1.5 py-0.5 border border-slate-200 outline-none rounded text-[10px] font-medium text-slate-800 bg-white"
                                    value={
                                      selectedRow?.recurEndFiscalYear || ""
                                    }
                                    onChange={(e) =>
                                      handleFieldChange(
                                        activeRecordId,
                                  ...
                                  <input
                                    ...
                                    className="w-full px-1.5 py-0.5 border border-slate-200 outline-none rounded text-[10px] font-medium text-slate-400 bg-slate-50 cursor-not-allowed"
                                    value={
                                      selectedRow?.recurEndEndingDate || ""
                                    }
                                    disabled
                                  />
                                </div>
                        ...
                        <FormSection title="Entry Info">
                          <FormInput
                            label="User"
                            value={
                              selectedRow?.entryUser || "15644.JAKIR.SHAIKH"
                            }
                            disabled
                          />
                          <FormInput
                            label="Date"
                            value={
                              selectedRow?.entryDate || "06/22/2026 01:02:05 PM"
                            }
                            disabled
                          />
                        </FormSection>
        ...
        <div className="voucher-secondary-trigger">
          <div className="flex items-center justify-between gap-4">
            ...
            <div className="flex items-center gap-2 shrink-0">
              <span className="mr-2 text-[10px] text-[#8a98ab] hidden md:inline">
                Last saved just now
              </span>
              <button
                type="button"
          ...
          <div className="voucher-second-screen">
            ...
            >
              <Toolbar
                ...
                buttonsDisable={["save"]}
                selectedRow={selectedChildRow}
                isDirty={(selectedRow?.detailLines || []).some(
                  (d) => d.isDirty,
                )}
                clipboardCount={childClipboard.length}
                clipboard={childClipboard}
                ...
                {!isChildFormView ? (
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="voucher-detail-content-shell">
                      {!selectedChildRow ? (
                        <div className="text-center text-gray-400 italic text-[11px] py-8">
                          No line selected. Add or select a line to edit
                          details.
                        </div>
                      ) : (
                        <>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end pt-1">
                                      <div className="md:col-span-1">
                                        <FormSearchSelect
                                          label="Organization"
                                          value={
                                            selectedChildRow.organization || ""
                                          }
                                          searchTerm={
                                            childSearchTerms.organization
                                          }
                                          setSearchTerm={(val) =>
                                            setChildSearchTerms((prev) => ({
                                              ...prev,
                                      ...
                                      <div className="md:col-span-2">
                                        <FormInput
                                          ...
                                          placeholder="Organization Name"
                                          value={
                                            selectedChildRow.organizationName ||
                                            ""
                                          }
                                          disabled
                                        />
                                      ...
                                      <div className="md:col-span-1">
                                        {projectDropdownOptions.length > 0 ? (
                                          <FormSearchSelect
                                            label="Project"
                                            value={
                                              selectedChildRow.project || ""
                                            }
                                            searchTerm={
                                              childSearchTerms.project
                                            }
                                            setSearchTerm={(val) =>
                                              setChildSearchTerms((prev) => ({
                                                ...prev,
                                  ...
                                  <FormSection title="Cost Calculations">
                                    ...
                                    <div className="grid grid-cols-3 gap-4 pt-1">
                                      <FormInput
                                        label="Tot Before Disc"
                                        value={
                                          selectedChildRow.totBeforeDisc || ""
                                        }
                                        disabled
                                      />
                                      <FormInput
                                  ...
                                  <FormSection title="Segment Abbreviations">
                                    <div className="grid grid-cols-1 gap-2">
                                      <FormSearchSelect
                                        label="Project Account"
                                        value={
                                          selectedChildRow.projAcctAbbrev || ""
                                        }
                                        searchTerm={
                                          childSearchTerms.projAcctAbbrev
                                        }
                                        setSearchTerm={(val) =>
                                          setChildSearchTerms((prev) => ({
                                            ...prev,
                                  ...
                                  <FormSection title="Sales/VAT Info">
                                    <div className="space-y-2">
                                      ...
                                      <FormInput
                                        ...
                                        type="number"
                                        value={
                                          selectedChildRow.salesVatTaxAmt || ""
                                        }
                                        disabled
                                      />
                                    </div>
                                  ...
                                  <FormSection title="Use/Reverse Info">
                                    <div className="space-y-2">
                                      <FormInput
                                        ...
                                        type="number"
                                        value={
                                          selectedChildRow.useReverseTaxAmt ||
                                          ""
                                        }
                                        disabled
                                      />
            ...
            <div className="mt-4 flex flex-col gap-4">
              {Object.keys(activeNestedTabs)
                ...
                .map((tabName) => {
                  if (!activeNestedTabs[tabName]) return null;
                  if (tabName === "Vendor Labor")
                    return renderVendorLaborContainer();
                  if (tabName === "Currency Line")
                    return renderCurrencyLineContainer();
                  if (tabName === "Customs Info")
                    return renderCustomsInfoContainer();
                  return null;
                })}
            </div>
          </div>
