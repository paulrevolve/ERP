...
import { MainContainer, Toolbar, SecondaryContainer } from "../helper/container";
import { ReusableTable } from "../helper/tableSection";
import { FormSection } from "../helper/formSection";
import api from "../utils/api";
import { backendUrl } from "./config";
...
const FormInput = ({
  ...
  if (isClickable) {
    return (
      <div className="space-x-2 flex items-center m-1">
        {label && (
          <label className="f-head font-[400] text-[10px] text-black min-w-[90px] cursor-pointer">
            {label} {required && "*"}
          </label>
        )}
        <input
          ...
          onChange={onChange}
          disabled={actualDisabled}
          className="w-3.5 h-3.5 rounded border-slate-300 cursor-pointer accent-[#17414d]"
        />
      </div>
    );
  }
  ...
  if (horizontal) {
    return (
      <div className={`space-x-2 flex items-center m-1 ${className}`}>
        {label && (
          <label className="f-head font-[400] text-[10px] text-black min-w-[90px] text-left select-none">
            {label} {required && "*"}
          </label>
        )}
        <input
          ...
          disabled={actualDisabled}
          placeholder={placeholder}
          className={`border outline-none p-0.5 rounded font-light text-[10px] transition-all flex-1 ${inputClassName}
            ${actualReadOnly || actualDisabled 
              ? "bg-gray-100 cursor-not-allowed text-gray-400 border-gray-200" 
              : "bg-white border-gray-300 text-black focus:border-[#17414d]"
            }`}
        />
      </div>
    );
  ...
  return (
    <div className={`flex flex-col gap-1 w-full m-1 ${className}`}>
      {label && (
        <span className="f-head font-[400] text-[10px] text-black select-none">
          {label} {required && "*"}
        </span>
      )}
      <input
        ...
        disabled={actualDisabled}
        placeholder={placeholder}
        className={`border outline-none p-0.5 rounded font-light text-[10px] transition-all w-full ${inputClassName}
          ${actualReadOnly || actualDisabled 
            ? "bg-gray-100 cursor-not-allowed text-gray-400 border-gray-200" 
            : "bg-white border-gray-300 text-black focus:border-[#17414d]"
          }`}
...
const FormSearchSelect = ({
  ...
  return (
    <div className="flex flex-col gap-1 w-full relative m-1">
      {label && (
        <span className="f-head font-[400] text-[10px] text-black select-none">
          {label}
        </span>
      ...
      <div className="relative w-full">
        <div className="relative group flex items-center">
          <input
            ...
            onFocus={() => !disabled && setShowDropdown(true)}
            className={`border outline-none w-full border-gray-300 pl-2 pr-8 py-0.5 rounded text-[10px] font-light transition-all
              ${disabled
                ? "bg-gray-100 cursor-not-allowed text-gray-400 border-gray-200"
            ...
            <div className="absolute left-0 top-full z-[100] w-full mt-1 bg-white border border-gray-300 rounded shadow-xl max-h-40 overflow-y-auto custom-scrollbar">
              {filteredOptions.length > 0 ? (
                filteredOptions.map((opt, idx) => (
                  <div
                    key={idx}
                    className="p-1.5 text-[10px] hover:bg-blue-50 cursor-pointer border-b border-gray-50 last:border-none font-normal text-black"
                    onClick={() => {
                      onSelect(opt);
                      setSearchVal("");
                      setShowDropdown(false);
                  ...
                  >
                    <span>{opt[displayKey]}</span>
                    {secondaryKey && opt[secondaryKey] && (
                      <span className="text-gray-400 ml-2">({opt[secondaryKey]})</span>
                    )}
                  </div>
                ))
              ) : (
              ...
              <div className="space-x-2 flex items-center m-1">
                ...
                <div className="relative flex-1">
                  <input
                    ...
                    className="border outline-none p-0.5 rounded font-light text-[10px] transition-all w-full border-gray-300 bg-white focus:border-[#17414d]"
                  />
                  <datalist id="taxabilityOptions">
                    <option value="T">T - Sales/VAT Taxable</option>
                    <option value="R">R - Reverse Charge / Use Tax</option>
          ...
          <div className="space-y-3">
            ...
            <FormSection title="Customs Line Information">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 py-2">
                ...
                <FormInput label="Value" value={selectedChildRow?.customsValue || ""} onChange={(e) => handleChildFieldChange(getRowKey(selectedChildRow), "customsValue", e.target.value)} />
                <div className="space-x-2 flex items-center m-1">
                  <label className="f-head font-[400] text-[10px] text-black min-w-[90px] select-none">
                    Goods/Services *
                  </label>
                  <select
                    className="border outline-none p-0.5 rounded font-light text-[10px] transition-all flex-1 border-gray-300 bg-white focus:border-[#17414d]"
                    value={selectedChildRow?.goodsServices || "G"}
                    onChange={(e) => handleChildFieldChange(getRowKey(selectedChildRow), "goodsServices", e.target.value)}
                  >
                    <option value="G">G</option>
                      ...
                      <div className="flex flex-col gap-3">
                        <FormSection title="Subcontractor Information">
                          ...
                          <div className="flex flex-col gap-1 w-full m-1">
                            <span className="f-head font-[400] text-[10px] text-black select-none">Invoice Type</span>
                            <select
                              className="border outline-none p-0.5 rounded font-light text-[10px] transition-all w-full border-gray-300 bg-white focus:border-[#17414d]"
                              value={selectedRow?.subcontractorInvoiceType || "None"}
                              onChange={(e) => handleFieldChange(activeRecordId, "subcontractorInvoiceType", e.target.value)}
                        ...
                        <FormSection className="flex-1 h-full" title="Notes">
                          <div className="space-y-1.5 flex flex-col h-full">
                            ...
                            <div className="space-y-1 flex-1 flex flex-col">
                              ...
                              <textarea
                                className="w-full flex-1 min-h-[140px] border outline-none p-1.5 rounded font-light text-[10px] bg-white border-gray-300 focus:border-[#17414d] transition-all resize-none"
                                value={selectedRow?.notesText || ""}
                                onChange={(e) => handleFieldChange(activeRecordId, "notesText", e.target.value)}
                              />
                            </div>
                ...
                {activeTab === "Payment" && (
                  <div className="space-y-3">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-1.5">
                      ...
                      <FormSection title="Transaction Rules">
                        <div className="flex flex-col gap-1 w-full m-1">
                          <span className="f-head font-[400] text-[10px] text-black select-none">Payment Transaction Type</span>
                          <select
                            className="border outline-none p-0.5 rounded font-light text-[10px] transition-all w-full border-gray-300 bg-white focus:border-[#17414d]"
                            value={selectedRow?.checkPayTransType || "None"}
                            onChange={(e) => handleFieldChange(activeRecordId, "checkPayTransType", e.target.value)}
                  ...
                  <>
                    ...
                    {activeChildTab === "Line Notes" && (
                      <div className="space-y-2">
                        ...
                        <textarea
                          className="w-full h-32 border outline-none p-2 rounded font-light text-[10px] transition-all border-gray-300 bg-white focus:border-[#17414d]"
                          placeholder="Line Notes..."
                          value={selectedChildRow.notes || ""}
                          onChange={(e) => handleChildFieldChange(getRowKey(selectedChildRow), "notes", e.target.value)}
                        />
